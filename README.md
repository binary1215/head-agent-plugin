...
This is a plugin-native reworking of the HEAD Agent Core design. It follows the packaging lesson from `oh-my-openagent`: one plugin namespace, a thin harness-facing surface, an isolated provider-neutral core, generated projections, and explicit capability gates. Version `0.3.0-alpha.43` establishes `PlatformAdapter`, `AgentRuntimeAdapter`, and `WorkspaceHostAdapter` boundaries for Codex and OpenCode, verifies current-host non-session interfaces, and binds a content-derived runtime-invocation authorization to the exact HEAD Project, Session, active Run, ExecutionContract, WholePlan, ContextCapsule, workspace mode, and resource limits. A host-local owner lock plus immutable project-lineage consumption and release receipts consume each authorization before child start and prevent sequential or concurrent replay with an honest at-most-once guarantee. The Codex one-shot adapter now requires protocol-evidence `0.2.0` to prove every fixed CLI option used by the actual invocation before consuming that lease, then composes the stable `codex exec --json --ephemeral` surface, exact authorized stdin, a host-local portable-subset output schema, privacy-reduced JSONL envelopes, bounded structured results, CLI execution/read, and read-only MCP retrieval through an integrity-verified native process supervisor. Provider-neutral validation retains the semantic byte and item limits after wire decoding. Durable transcript-free invocation records and the exactly-once Run-result application bridge live in a provider-neutral core: record recovery verifies the authorization, runtime, project, Session, scope, receipt, events, and draft as one lineage, while canonical ResultPacket evidence derives its runtime from that verified authorization rather than from Codex-specific code. Windows Job Objects and POSIX process groups provide the same descendant-tree cleanup contract; Windows normal-exit/cancellation, both provider-neutral lifecycle identities, the OpenCode protocol-fixture mode, the supervised Codex protocol fixture, pre-consumption CLI-surface drift rejection, portable wire-schema enforcement, and privacy-reduced provider diagnostics are verified. The live Codex Session and consequential Run now both conform: the Run invoked the real provider, performed the exact isolated file write, verified native descendant cleanup, applied its structured draft to one canonical ResultPacket, and completed Fresh HEAD review without mutating Product Canon or persisting a raw transcript. The application bridge mirrors Execution Lineage text normalization so harmless provider whitespace cannot conflict with its canonical projection. A deterministic 160 KiB Codex tool-event fixture retains coverage of the former 128 KiB failure shape. The live harness defaults to the efficient `run-only` mode and retains `session-and-run` as an optional fuller regression; it requires deliberate live opt-in but does not impose a product-level model-call count. The transport guard now allows 8 MiB total stdout and 2 MiB per JSONL event, while the independent structured ResultPacket contract remains 128 KiB. OpenCode actual invocation, provider resume/attachment, and general start/stream/interrupt/close control remain disabled.
