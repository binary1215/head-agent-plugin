...
This is a plugin-native reworking of the HEAD Agent Core design. It follows the packaging lesson from `oh-my-openagent`: one plugin namespace, a thin harness-facing surface, an isolated provider-neutral core, generated projections, and explicit capability gates. Version `0.3.0-alpha.44` adds a content-derived, user-selected repository source scope to onboarding and World Model scans, ranks product candidates by behavior evidence instead of alphabetical symbol order, and bounds Context expansion to semantic adjacency plus one deterministic temporal anchor. A 291-file isolated NeoPick projection reproduced the same Capsule in 6.5–7.2 seconds after the prior broad path did not finish in roughly three minutes. Scope and inference remain evidence-only; no candidate is promoted automatically. The existing provider-neutral runtime authorization, at-most-once lease, native process-tree supervision, transcript-free result path, live Codex Session/Run conformance, 8 MiB stdout envelope, 2 MiB event envelope, and independent 128 KiB structured-result contract remain intact. OpenCode actual invocation, provider resume/attachment, and general start/stream/interrupt/close control remain disabled.
