...
This is a plugin-native reworking of the HEAD Agent Core design. It follows the packaging lesson from `oh-my-openagent`: one plugin namespace, a thin harness-facing surface, an isolated provider-neutral core, generated projections, and explicit capability gates. Version `0.3.0-alpha.34` establishes `PlatformAdapter`, `AgentRuntimeAdapter`, and `WorkspaceHostAdapter` boundaries for Codex and OpenCode, verifies current-host non-session interfaces, and adds a content-derived runtime-invocation authorization bound to the exact HEAD Project, Session, active Run, ExecutionContract, WholePlan, ContextCapsule, workspace mode, and resource limits. A no-descendant conformance fixture proves bounded stdin, privacy-reduced JSONL event envelopes, exact-child exit, timeout and caller-cancellation cleanup, and transcript-free ResultPacket drafts for both runtimes. Actual provider invocation, provider-session attachment, execution leases, and every start/resume/stream/interrupt/close control remain disabled.
