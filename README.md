...
This is a plugin-native reworking of the HEAD Agent Core design. It follows the packaging lesson from `oh-my-openagent`: one plugin namespace, a thin harness-facing surface, an isolated provider-neutral core, generated projections, and explicit capability gates. Version `0.3.0-alpha.33` establishes `PlatformAdapter`, `AgentRuntimeAdapter`, and `WorkspaceHostAdapter` boundaries for Codex and OpenCode across a Windows/macOS/Linux contract matrix, adds current-host read-only CLI discovery, and proves bounded non-session version plus provider-specific protocol/capability surfaces through exact direct child processes. Those observations bind to canonical HEAD project and Session identities without passing project content or creating provider sessions. Returned evidence contains only allowlisted capability signals, content-derived path/output identities, normalized versions, and lifecycle facts—never raw paths, raw commands, raw output, credentials, provider sessions, or PIDs. Every start, resume, stream, interrupt, close, host, and process-control operation remains disabled until an accepted ExecutionContract, caller and process-tree ownership, bounded event/result schemas, lifecycle recovery, and authority conformance are proved.
