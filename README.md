...
This is a plugin-native reworking of the HEAD Agent Core design. It follows the packaging lesson from `oh-my-openagent`: one plugin namespace, a thin harness-facing surface, an isolated provider-neutral core, generated projections, and explicit capability gates. Version `0.3.0-alpha.29` adds snapshot-scoped ArcadeDB `TRAVERSE` expansion behind a conformance-gated transport contract. GraphDB remains optional and non-authoritative: the client fixes anchors, policy, bounds, ordering, and semantic digests, verifies the complete bounded server radius against the immutable GraphSnapshot, and rejects missing, forged, duplicate, stale, out-of-radius, or truncated responses before returning the deterministic reference result.
