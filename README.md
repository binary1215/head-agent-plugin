...
This is a plugin-native reworking of the HEAD Agent Core design. It follows the packaging lesson from `oh-my-openagent`: one plugin namespace, a thin harness-facing surface, an isolated provider-neutral core, generated projections, and explicit capability gates. Version `0.3.0-alpha.10` adds user-owned Product Model canon and its Git-independent temporal projection with immutable FeatureGroup, Capability, Feature, Requirement, Constraint, and Decision revisions on top of the evidence-linked Execution Lineage, Repository World Model, and deterministic Context Compiler.
