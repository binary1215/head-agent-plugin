...
This is a plugin-native reworking of the HEAD Agent Core design. It follows the packaging lesson from `oh-my-openagent`: one plugin namespace, a thin harness-facing surface, an isolated provider-neutral core, generated projections, and explicit capability gates. Version `0.3.0-alpha.30` adds a provider-neutral prepared-traversal verification contract. An explicitly re-conformed ArcadeDB activation now reads only the current pointer, topology manifest, and bounded traversal radius during query execution—never the full remote snapshot or topology—while the embedded GraphSnapshot still fixes anchors, policy, bounds, ordering, and semantic result digests. GraphDB remains optional, derived, and non-authoritative.
