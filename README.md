...
This is a plugin-native reworking of the HEAD Agent Core design. It follows the packaging lesson from `oh-my-openagent`: one plugin namespace, a thin harness-facing surface, an isolated provider-neutral core, generated projections, and explicit capability gates. Version `0.3.0-alpha.20` adds the provider-neutral `GraphProjectionAdapter` boundary with local JSON and in-memory conformance implementations. Temporal CLI/MCP queries and Context Compiler expansion preserve the same deterministic result identity across adapters, fall back to the embedded recoverable GraphSnapshot when no projection exists, and reject stale or authority-claiming backends. GraphDB remains optional and unimplemented. The conformant Go `repository.scan.v1` candidate remains non-default because current benchmark evidence does not justify production selection.
