...
This is a plugin-native reworking of the HEAD Agent Core design. It follows the packaging lesson from `oh-my-openagent`: one plugin namespace, a thin harness-facing surface, an isolated provider-neutral core, generated projections, and explicit capability gates. Version `0.3.0-alpha.32` establishes `PlatformAdapter`, `AgentRuntimeAdapter`, and `WorkspaceHostAdapter` boundaries for Codex and OpenCode across a Windows/macOS/Linux contract matrix, adds current-host read-only CLI discovery, and proves a bounded non-session version surface through exact direct child processes. Discovery and version evidence expose only content-derived path/output identities, normalized versions, and lifecycle facts—never raw paths, raw output, project content, credentials, or provider sessions. Every start, resume, stream, interrupt, close, host, and process-control operation remains disabled until project/session binding, caller and process-tree ownership, lifecycle recovery, and authority conformance are proved.
