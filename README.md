...
This is a plugin-native reworking of the HEAD Agent Core design. It follows the packaging lesson from `oh-my-openagent`: one plugin namespace, a thin harness-facing surface, an isolated provider-neutral core, generated projections, and explicit capability gates. Version `0.3.0-alpha.35` establishes `PlatformAdapter`, `AgentRuntimeAdapter`, and `WorkspaceHostAdapter` boundaries for Codex and OpenCode, verifies current-host non-session interfaces, and binds a content-derived runtime-invocation authorization to the exact HEAD Project, Session, active Run, ExecutionContract, WholePlan, ContextCapsule, workspace mode, and resource limits. A durable owner lock plus immutable consumption and release receipts consume each authorization before child start and prevent sequential or concurrent replay with an honest at-most-once guarantee. The no-descendant fixture proves bounded stdin, privacy-reduced JSONL event envelopes, exact-child exit, timeout and caller-cancellation cleanup, and transcript-free ResultPacket drafts for both runtimes. Actual provider invocation, provider-session attachment, live descendant ownership, and every start/resume/stream/interrupt/close control remain disabled.
...
- Provider-neutral JSONL event envelopes, lifecycle receipts, and ResultPacket drafts that omit raw transcripts, commands, paths, provider-session identifiers, and PIDs; a fixed no-descendant fixture proves Codex/OpenCode success and timeout cleanup without invoking either provider.
