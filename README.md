...
This is a plugin-native reworking of the HEAD Agent Core design. It follows the packaging lesson from `oh-my-openagent`: one plugin namespace, a thin harness-facing surface, an isolated provider-neutral core, generated projections, and explicit capability gates. Version `0.3.0-alpha.31` adds content-derived prepared-traversal cost evidence and a read-only live benchmark path. The reviewed 64-file fixture records 20,478 normalized prepared-query bytes versus an 833,590-byte full-reload baseline, a deterministic 97.54% reduction; elapsed time and transport observations remain non-semantic diagnostics. A live run requires an already verified prepared ArcadeDB activation and environment-resolved credentials, accepts no credential flags, and forbids every remote write operation.
