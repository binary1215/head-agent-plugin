...
`.head/context/product-model.json`의 Product Model 소스는 사용자가 소유하는 Canon입니다. 저장소 디렉터리와 독립적으로 안정적인 FeatureGroup, Capability, Feature, Requirement, Constraint 및 Decision 키를 정의합니다. 선택적 schema-v2 Policy record는 불변 P3 candidate와 정확한 user ReviewDecision을 거쳐 명시적인 Feature 또는 FeatureGroup key에만 적용됩니다. Group membership은 inheritance를 만들지 않으며 graph는 두 번째 Policy store가 아닙니다. Schema-v1 byte와 identity는 정확히 유지됩니다. 이전에 초기화된 프로젝트에서 Product Model이 누락된 경우 이는 명시적으로 비어 있는 semantic model입니다. 코드와 문서가 자동으로 채우지 않습니다. [`product-model.md`](product-model.md)를 참조하세요.

temporal provenance graph는 별도의 검증된 프로젝션입니다. traversal protocol은 `lexical-discovery`와 exact HEAD anchor mode를 상호 배타적으로 제공합니다. exact mode는 현재 GraphSnapshot과 1~32개의 node ID에 결속되고 relation·authority·freshness·confidence·depth·node·edge·candidate policy를 확대할 수 없습니다. Context Compiler는 HEAD 소유 EvidenceNeed 안의 exact anchor만 받아 별도 `GraphTraversalEvidence`를 만들며 lexical search를 semantic relevance로 승격하지 않습니다. Product relation은 `canon-projected`이고 candidate-space node는 명시적 opt-in 없이 숨겨지며, local/ArcadeDB projection은 embedded GraphSnapshot을 복구 소스로 보존합니다. Policy approval path는 무관한 Product Model revision 뒤에도 변경되지 않은 current Policy revision과 연결됩니다. review된 source change와 product impact는 exact revision에 결속되며 Git ref만으로 deployment를 추론하지 않습니다.

Graph 계보는 두 번째 이력 저장소를 만들지 않고 보존된 content-addressed World snapshot을 재사용합니다. `GraphLineageStatusProjection`, `GraphLineageTraceProjection`, `GraphLineageDiffProjection`은 bounded Hot/Warm/Cold page, exact 또는 discovery trace, 검증된 execution artifact reference, Policy-evidence byte currentness, snapshot diff를 제공하는 비지속 P4 view입니다. 이들은 P2 recovery direction을 쓰거나 accepted Policy를 무효화하거나 semantic continuity를 추론할 수 없습니다. 제거·추가된 path 사이의 동일 content digest는 `exact-content-possible-move` evidence만 만들며, semantic identity 주장은 별도 HEAD proposal이고 실제 승격할 때만 review가 필요합니다.

공급자 중립 공통 Observation 경계는 Core에 제품 어휘를 가르치지 않고 domain-shaped data를 기록합니다. `ObservationTypeDescriptor`, `ObservationRecord`, `DerivedObservationRecord`, `ObservationCollectionReceipt`는 P3이고, `ObservationStatusProjection`은 별도의 재구축 가능한 P4 evidence graph이며 검증된 node와 relation은 unified temporal GraphSnapshot에도 projection됩니다. `ObservationSourceBinding`은 P5 Host configuration입니다. process-local `ObservationAdapterRegistry`는 source별 optional adapter를 하나의 정확한 ready Project에 binding하면서 path, credential, cursor 또는 provider identity를 persist하지 않습니다. 비지속 discovery view는 registration gate 없이 exact filter, disclosed read-only resynchronization을 포함한 opaque pagination, bounded descriptor shape, Host-local availability hint를 제공합니다. `ObservationPreparationProjection`은 reuse-first UX를 위해 exact existing-evidence query와 matching source discovery를 결합하지만 meaning, sufficiency, source 또는 collection을 선택하지 않습니다. Host-injected MCP composition은 opaque Project-bound source ID로만 collect합니다. `JsonEventFileObservationAdapter`는 MCP file-path surface를 추가하지 않는 bounded one-shot CI reference를 제공합니다. 자동 graph relation은 `CONFORMS_TO`, `EVIDENCED_BY`, `DERIVED_FROM`으로 제한됩니다. Metric workflow는 같은 P3 record를 재사용하고, numeric comparison을 막지 않으면서 adapter-revision 및 다른 collection condition difference를 보여주며, assessment는 non-causal ProductHypothesis와 선택적인 review-gated initiative candidate로 유지합니다. Context Compiler는 정확한 HEAD 소유 EvidenceNeed를 통해서만 observation을 포함합니다. observation collection은 Product Canon, product meaning, ReviewDecision 또는 P2 recovery direction을 만들 수 없습니다. 잘못된 Observation lineage는 해당 graph layer만 unavailable로 만들며 관련 없는 World/Product 작업은 계속 사용할 수 있습니다. [`observation-adapters.md`](observation-adapters.md)를 참고하세요.

비차단 Conformance 재정은 공급자 HEAD가 정확히 승인된 Canon을 현재 source, ChangeSet, Observation 또는 선택적 Graph evidence와 비교하게 합니다. Core는 anchor를 검증하고 P3 candidate, resolution candidate, exact-Finding 사용자 disposition만 지속하며 lexical 또는 semantic verdict를 수행하지 않습니다. paginated P4 queue와 candidate-hidden audit graph는 일반 작업을 막거나 fix를 승인하거나 Canon을 개정하거나 recovery를 쓸 수 없습니다. Graph 부재, partial coverage, 선택적 adapter 손실, advisory risk는 disclosure로 남습니다. process-local 선택적 P5 trigger registry는 검증된 Host evidence를 coalesce하고 uncertain provider outcome 뒤 automatic replay를 중단합니다. 기본 opportunistic 사용은 background provider를 호출하지 않습니다. [`conformance-reconciliation.md`](conformance-reconciliation.md)를 참고하세요.
