...
Selection gain and additional-budget estimates count only new evidence IDs for
each unmet need. A duplicate relation carrier cannot inflate either count, but
it remains eligible when it fulfills an independent repository/source need.
Alternative carriers remain visible in the proof; this is set accounting, not
a semantic relevance judgment or a new execution gate.

Product coverage counts verified Project, Product Model, logical entity (kind and
key), and revision identities, not the number of ProductContext carriers. Both
exact-key and facet-only needs use that identity for selection gain, coverage,
and budget estimates; repeated carriers retain separate provenance but cannot
inflate the count. Exact HEAD-requested Product keys are resolved before optional
...
Facet coverage checks the actual content fields of each evidence kind, not its
query, selector, provenance, or diagnostic metadata. Those records remain in the
Capsule for audit, but a requested word echoed in a traversal query cannot prove
its own inclusion. The same evidence content and facets are assessed alike
whether discovered lexically or requested by exact key; neither path decides
semantic relevance or sufficiency.
...
The common Capsule reader verifies the content digest first and then binds `snapshot.projectId` to the current logical HEAD Project. An intact Capsule copied from another Project therefore fails with `CONTEXT_CAPSULE_PROJECT_MISMATCH` before an ExecutionContract or Run can consume it. This check deliberately does not compare a filesystem root string, HEAD Session ID, current World identity, or freshness: historical Capsules remain reusable under the same logical Project when the later execution contract permits them, while folder or provider changes do not become authority.
