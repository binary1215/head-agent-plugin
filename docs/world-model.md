...
Debounced filesystem and CI trigger ingestion, single-writer event coalescing, automatic GraphDB refresh, and automatic document regeneration are deferred. The explicit refresh contract is the only active mutation path for incremental observed state.
