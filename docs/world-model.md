...
The repository scan is the first built-in `ComputeAdapter` operation. Its semantic output contains only normalized relative paths and deterministic source facts. The scan root is operational input and is excluded from the repository-scan result; backend identity, execution mode, timing, and process details remain pointer diagnostics outside World Model identity. The default adapter now probes the verified Go distribution first, but the release worker does not yet advertise `repository.scan.v1`, so the JavaScript reference performs the scan and the pointer discloses the fallback reason. The World Model continues to record its separately validated canonical project root. A candidate native implementation must produce a byte-equivalent canonical response on the tracked corpus before it can replace the JavaScript operation.
...
- the Go implementation of `repository.scan.v1` and benchmark-gated migration of graph/traversal/Context operations;
