...
Debounced filesystem and CI trigger ingestion plus single-writer event coalescing are active and both terminate in the same incremental refresh mutation path. Background service installation, provider-specific CI webhook adapters, automatic GraphDB refresh, and automatic document regeneration remain deferred.
