...
World Model version `0.13.0` includes repository scan protocol `0.4.0`, repository source-scope protocol `0.1.0`, source-analysis protocol `0.2.0`, semantic graph protocol `0.2.0`, onboarding projection protocol `0.1.0`, Feature mapping protocol `0.2.0`, ChangeSet protocol `0.1.0`, ChangeSet projection protocol `0.2.0`, document-change graph projection protocol `0.1.0`, Product Operating Loop protocol `0.2.0` with `0.1.0` artifact read compatibility, VCS evidence protocol `0.1.0`, GraphProjectionAdapter `0.1.0`, incremental refresh protocol `0.2.0`, refresh-trigger protocol `0.2.0`, and a separate P4 `GraphSnapshot` built by temporal provenance protocol `0.10.0`. Version `0.13.0` replaces raw ref-file hashing with canonical product-ref state and excludes the declared Host-operational namespace. The P4 boundary is verified through the content-derived AuthorityPlaneContract, and graph materialization is fenced from Product Canon bytes. World Model `0.11.0` and `0.12.0` remain digest- and cross-projection-verifiable but become stale until rebuilt under the current indexer contract. Legacy repository scan `0.2.0` and `0.3.0` snapshots remain digest-verifiable and become stale until rebuilt under the new indexer contract. Repository scan `0.4.0` excludes common Python runtime and cache directories from eligible product evidence while preserving user-selected source scope. The temporal graph adds immutable onboarding history, Feature/code/test mapping candidates, reviewed provider-neutral ChangeSets, exact revision deltas, review-gated Feature/Capability impact, optional ChangeSet-to-Git evidence, document review/application audit lineage, and epistemically typed product-operating evidence. Non-persisted product notes never enter the World Model. A v0.2 Initiative candidate may use inline reasoning and defer Feature resolution until its explicit review; the separate reviewed Initiative carries the exact resolution. Candidate surfaces remain hidden by default; only scoped explicit ReviewDecisions create reviewed receipts or canonical relations. Document ReviewDecisions remain P1 normative records, application receipts remain immutable P3 evidence, and their graph nodes remain P4 derived evidence. Incremental refresh and trigger ingestion change no snapshot schema; batching, timing, writer ownership, reuse, and adapter diagnostics stay outside World Model and GraphSnapshot semantic identity. Legacy refresh `0.1.0` artifacts remain readable.
