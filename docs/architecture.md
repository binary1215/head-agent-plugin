...
`ComputeAdapter` contract `0.2.0` and WorkerProtocol `0.2.0` define a replaceable computation boundary without changing semantic identities or transferring authority from the JavaScript control plane. Canonical requests bind a versioned operation, input digest, semantic producer, and resource limits. Canonical responses are all-or-nothing, digest-verified, size-bounded, structured, and fixed to `authorityEffect: none`.

`JsReferenceComputeAdapter` is the only active backend and the conformance oracle. It runs in process and does not mutate project state or canon. The first active built-in operation, `repository.scan.v1`, emits relative-path-only file and source facts that are strictly validated before World Model consumption. Its tracked corpus, failure cases, benchmark harness, and complete-response conformance gate define what a Go implementation must preserve. Compute-backed graph operations, the Go worker, binary integrity selection, native process supervision, and fallback remain deferred. See [`compute-adapter.md`](compute-adapter.md).
