...
The provider-neutral common Observation boundary records domain-shaped data without teaching Core product vocabulary. `ObservationTypeDescriptor`, `ObservationRecord`, `DerivedObservationRecord`, and `ObservationCollectionReceipt` are P3; `ObservationStatusProjection` is a separate rebuildable P4 evidence graph; `ObservationSourceBinding` is P5 Host configuration. Automatic graph relations are limited to `CONFORMS_TO`, `EVIDENCED_BY`, and `DERIVED_FROM`. Context Compiler includes an observation only through an exact HEAD-owned EvidenceNeed; observation collection cannot create Product Canon, product meaning, a ReviewDecision, or P2 recovery direction. See [`observation-adapters.md`](observation-adapters.md).
