...
The first runtime layer defines explicit `PlatformAdapter`, `AgentRuntimeAdapter`, and `WorkspaceHostAdapter` boundaries. Codex and OpenCode plus Windows, macOS, Linux, and the native-process host are represented by deterministic contract artifacts. A current-host machine-discovery composition inspects absolute PATH entries and regular executable candidates without launching them, exposing only content-derived path identities, byte size, launcher kind, and symlink/direct-spawn safety rather than raw paths. A bounded version-evidence composition directly invokes only a native non-symlink candidate with a fixed version flag, and a provider-specific protocol composition invokes only fixed help profiles. Both use no shell, ignored stdin, a minimal environment, bounded output/timeout, exact-child cleanup, and no provider session; raw paths, raw commands, raw output, and project content remain excluded. A separate `RuntimeProjectBinding` connects the observations to canonical HEAD project and Session identities while reducing the physical root to a digest. Every launch, resume, stream, interrupt, close, messaging, and process-control method still fails closed. Only a later adapter that proves an accepted ExecutionContract, caller and process-tree ownership, bounded event/result schemas, lifecycle recovery, actual provider-session evidence, and authority conformance may activate runtime control. See [`runtime-adapters.md`](runtime-adapters.md).
