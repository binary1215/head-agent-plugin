...
The first runtime layer defines explicit `PlatformAdapter`, `AgentRuntimeAdapter`, and `WorkspaceHostAdapter` boundaries. Codex and OpenCode plus Windows, macOS, Linux, and the native-process host are represented by deterministic contract artifacts. A current-host machine-discovery composition inspects absolute PATH entries and regular executable candidates without launching them, while bounded version and provider-specific help compositions prove non-session interfaces. `RuntimeProjectBinding` connects those observations to canonical HEAD project and Session identities. A separate immutable `RuntimeInvocationAuthorization` now requires the exact active Run, verified ExecutionContract/WholePlan/ContextCapsule, explicit runtime/workspace allowed actions, project-root digest, and bounded resources. An operational owner lock serializes use, an immutable consumption receipt burns the authorization before child start, and a release receipt records exact lock cleanup without claiming exactly-once execution. JSONL `RuntimeEventEnvelope`, lifecycle receipt, and transcript-free ResultPacket-draft contracts bind those lease receipts through a fixed no-descendant fixture for both runtime identities, including timeout cleanup. Actual provider execution, live process-tree fencing, provider-session attachment, and all runtime controls remain disabled. See [`runtime-adapters.md`](runtime-adapters.md).
...
`RuntimeStateAdapter` supplies strict point-in-time external runtime observations. Its source descriptor is pointer metadata rather than semantic identity; normalized observations are content-addressed and freshness-gated. The adapter is read-only and grants neither instruction nor runtime control authority. Separate `PlatformAdapter`, `AgentRuntimeAdapter`, and `WorkspaceHostAdapter` references make provider/platform/host composition explicit. Current-host discovery, bounded fixed version/help evidence, and `RuntimeProjectBinding` establish capability without authorization. The invocation-authorization layer then derives a non-executing single-call boundary from the exact active HEAD lineage; a durable at-most-once lease consumes it before the conformance child starts and keeps it non-replayable after completion, timeout, cancellation, or caller failure. Event/receipt/draft schemas and a no-descendant fixture prove the control-plane shape without invoking a provider. Actual provider-session binding, hydration, launch/resume/stream/interrupt/close, messaging, live caller/process-tree fencing, and process control remain deferred. AST-accurate graphs and structured Git decision inference also remain deferred. See [`runtime-state.md`](runtime-state.md) and [`runtime-adapters.md`](runtime-adapters.md).
