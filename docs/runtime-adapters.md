...
2. diagnose and fix the consequential live Codex Run `invalid-event`, then validate actual provider input, structured events, isolated file write, ResultPacket evidence, and provider-specific errors through the provider-neutral schemas under a separately approved retry;
