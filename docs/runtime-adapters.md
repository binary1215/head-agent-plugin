...
2. validate the diagnosed large-event fix against one separately approved consequential live Codex Run, including actual provider input, structured events, isolated file write, ResultPacket evidence, and provider-specific errors through the provider-neutral schemas;
