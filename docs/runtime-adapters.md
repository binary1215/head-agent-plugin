...
Runtime-adapter contract `0.1.0` establishes the v0.6 provider-neutral boundary. Runtime-machine-discovery protocol `0.1.0` adds current-host read-only executable discovery, runtime-version-evidence protocol `0.1.0` adds a bounded non-session direct version invocation, runtime-protocol-evidence protocol `0.2.0` observes fixed provider-specific help surfaces and the exact Codex one-shot option set, and runtime-project-binding protocol `0.1.0` binds those observations to canonical HEAD project and Session identities. Execution-authorization protocol `0.2.0` adds one envelope with `scope.kind: session | run`; execution-lease protocol `0.3.0` separates durable consumption/release evidence from operational owner state; process-supervisor protocol/manifest `0.1.0`, event-envelope `0.1.0`, structured-result `0.1.0`, lifecycle-receipt `0.5.0`, and ResultPacket-draft `0.4.0` carry scope through the common lifecycle boundary. A bounded Codex one-shot composition and its native descendant-tree supervisor are implemented, while successful live model conformance, general runtime controls, and OpenCode execution remain disabled.
...
1. execute one bounded live Codex Session through the externalized operational-state root, exact authorization/lease/caller/project fences, and the verified native descendant supervisor;
2. execute one consequential live Codex Run and validate actual provider input, structured events, ResultPacket evidence, and provider-specific errors through the provider-neutral schemas;
