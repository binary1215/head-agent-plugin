...
Current milestone: artifact-only Session restore and reviewed worker-result checkpoint integration
...
Immediate next slice: publish the verified provider-neutral HF-008/HF-009/HF-010 outcome implementation and return the exact commit to the original author for source audit. Protocol 0.3 checkpoints freeze an immutable Session pointer; `SessionRestoreProjection` reproduces current consumer input from verified artifacts without provider resume; and only an accepted exact Fresh HEAD ReviewDecision plus one create-only integration request may connect one reviewed ResultPacket to one explicit recovery checkpoint. Legacy checkpoints remain audit-readable but cannot drive current restore. Incorporate any blocking correction, then prove the hostless resident-HEAD consumer outcome. Preserve the already-passing Codex/OpenCode attachment, replacement, reply-wait, interrupt, and close evidence and the final three-verdict requirement.
