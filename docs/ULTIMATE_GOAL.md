...
Immediate next slice: complete direct original-author source audit of the now-passing actual Codex/OpenCode provider-client composition over the provider-neutral live-coordination vertical. Host-issued role bindings, Project/HEAD Session/generation fences, durable host-local inbox/read/reply/idempotency state, append-only binding-scoped endpoint targets, fresh-snapshot caller/target verification, exact delivery acknowledgment, post-send topology verification, and delivery-separated non-authoritative receipts are implemented. A production `host-export` adapter reaches a provider-neutral offline wake boundary without Herdr: an external host publishes content-verified endpoint snapshots whose unique endpoint tuple is bound to the current coordination `bindingId` and a host-issued per-process proof hash. A sender that owns its exact binding/proof may resolve the current recipient binding to one unique offline registered endpoint without granting that endpoint role or process authority, create a request bound to that recipient binding, and wait while the host acquires a create-only pre-effect claim. The host then starts the actual recipient provider with its distinct raw proof and emits the exact wake acknowledgment as soon as that owned process start is observed; delivery no longer waits for or implies a reply. The recipient's first MCP call independently revalidates proof possession and current binding ownership before reading, and any immutable reply remains a separate durable message. Copied endpoint coordinates, a different or replaced binding, a forged/old proof, duplicate ownership, explicit detach, and stale topology fail closed. Actual OpenCode `send` and newly started Codex `read-inbox`/`reply` calls now pass this full path under native descendant-tree supervision, preserve byte-identical `.head`, and persist neither raw proofs/binding tokens nor provider-session references. The original already-live-target behavior remains distinct from this offline-wake extension and continues through the exact attachment path. Herdr-specific executable, socket, CLI-command, pane, and TUI integration remains intentionally outside this plugin. Provider-session resume, general stream/interrupt/close, broader process-host control, shell interpretation, TUI scraping, automatic merge, automatic product-canon promotion, autonomous success judgment, universal-directory publication, and the still-required author source verdict remain separate work.
...
| durable role coordination | host-issued bindings, authority generation, durable inbox/read/reply/idempotency state, three role-bound MCP operations, append-only endpoint targets, and delivery-separated receipts are implemented host-locally | retain the non-authoritative Core contract; satisfy live delivery through the generic host-driver boundary or a separately owned optional adapter without importing host-specific identity into Core |
