...
Immediate next slice: complete direct original-author source audit and an actual Codex/OpenCode provider-client composition over the provider-neutral live-coordination vertical. Host-issued role bindings, Project/HEAD Session/generation fences, durable host-local inbox/read/reply/idempotency state, append-only binding-scoped endpoint targets, fresh-snapshot caller/target verification, exact delivery acknowledgment, post-send topology verification, and delivery-separated non-authoritative receipts are implemented. A production `host-export` adapter now reaches the same wake-delivery boundary without Herdr: an external host publishes content-verified endpoint snapshots whose unique endpoint tuple is bound to the current coordination `bindingId` and a host-issued per-process proof hash, injects the raw proof only into that role process, acquires a create-only pre-effect claim for a create-only filesystem delivery request, and returns an exact create-only acknowledgment. Every fresh snapshot revalidates proof possession and binding ownership; copied endpoint coordinates, a different binding, a forged proof, or duplicate ownership fail closed. An unacknowledged claim is ambiguous and cannot be automatically consumed again. Two fresh role-bound MCP processes plus a separate live host consumer pass this path. This is real external-host bridge evidence, but it does not yet prove that actual Codex and OpenCode provider clients consume the wake and perform the role-tool round trip. Herdr-specific executable, socket, CLI-command, pane, and TUI integration remains intentionally outside this plugin. Provider-session resume, general stream/interrupt/close, broader process-host control, shell interpretation, TUI scraping, automatic merge, automatic product-canon promotion, autonomous success judgment, and universal-directory publication remain separate optional work.
...
| live role coordination across workspace hosts | durable role state, exact endpoint targets, per-process proof plus current-binding ownership, a production host-export filesystem bridge, two fresh role-bound MCP processes, and a separate live host consumer pass without Herdr or project mutation; actual Codex/OpenCode provider clients have not yet consumed the wake and completed the role-tool round trip | **partial** |
