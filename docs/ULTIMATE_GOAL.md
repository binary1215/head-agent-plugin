...
Current milestone: hostless resident-HEAD recovery consumption and exact original Feature mapping

Previous completed slice: the five-plane authority contract made the Graph/record boundary executable, received direct original-author alignment and `philosophical superiority: yes`, and then closed the author's two non-blocking Feature-candidate and P3/P4/P5-to-P2 counterexamples in pushed commit `a1e32f7`.

Immediate next slice: publish the verified hostless resident-HEAD recovery proof and return the exact commit to the original author for direct source audit. Preserve the original numbering exactly: HF-007 is context-compaction continuity, HF-008 is prior HEAD Session restore, HF-009 is independently ownable worker dispatch, and HF-010 is completed worker integration. The current artifact-only restore is a provider-neutral semantic equivalent for HF-008, not a claim to restore an OpenCode `ses_*` conversation; the reviewed-result checkpoint transaction is an HF-010 integration equivalent, not HF-009 dispatch. The separate runtime and coordination vertical carries bounded dispatch evidence. Require identical consumer direction across fresh Codex/OpenCode processes, request/checkpoint crash convergence, missing-P3 direction preservation, concurrent integration convergence, non-accept rejection, and one resident HEAD next move that ignores inbox text as authority. Preserve the final three-verdict requirement.
