...
Immediate next slice: diagnose and fix the bounded live Codex Run `invalid-event` without repeating a model call, then request separate approval for one consequential Run retry through the established `ExecutionAuthorization`, external operational-state boundary, durable at-most-once lease, native descendant-tree ownership, JSONL event normalization, structured result, isolated workspace write, ResultPacket application, and Fresh HEAD transition. The approved post-hardening live Session has conformed successfully. The following approved live Run invoked Codex and observed a structured result but ended before `turn.completed`; therefore file-write verification and canonical application remain unproved. The deterministic Run-draft application path accepts only a completed, native-supervised actual-provider Run and remains fail closed. The integrated ArcadeDB activation and prepared-traversal evaluation remains a separate optional `sandbox` E2E that requires explicit risk-informed external-write approval. Provider-session resume, general stream/interrupt/close, messaging, shell interpretation, and TUI scraping remain disabled until Run lifecycle conformance is proved.
