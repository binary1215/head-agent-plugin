...
Status: repository-development goal and append-only work history

> Development-only boundary
>
> This document guides maintainers who are changing this plugin and records the
> evidence, milestones, experiments, and comparative audits used during its
> development. It is not Product Canon, project direction, runtime instruction,
> a Context Compiler source, WholePlan authority, recovery authority, or an
> installed-plugin operating input. The distribution builder excludes it from
> releases. A target project must derive direction only from the current user
> request, verified project Canon, current Session/Run recovery state, and
> explicit ReviewDecisions. Named projects, databases, repositories, benchmark
> corpora, and candidate sets below are historical validation evidence only and
> must never be inferred as defaults or reusable inputs for another project.

Current milestone: completed source audit and unconditional final superiority verdict for the portable continuation and independently ownable worker vertical

Previous completed slice: pushed commit `e71b151` proved hostless resident-HEAD recovery across independent processes, crash-shape and concurrency convergence, missing-P3 direction preservation, non-accept rejection, and one authority-safe resident next move. The original author directly audited that source, confirmed original-philosophy and P1/P2/P3/P4 Graph/record alignment with `blocking correction: none`, and retained `philosophical superiority: yes` while withholding the two overall verdicts for portable continuation and independently ownable worker evidence.

Immediate next slice: no correction remains for this comparative milestone. Preserve the audited HF-007/008/009/010 mapping, P1/P2/P3/P5 boundaries, provider-neutral Host adapter design, optional resume/stream capability, and Herdr-independent Core. Any later feature work starts from a new user-owned objective and must not reopen capabilities the governing behavioral-completeness criterion classified as optional or ritual parity without new consumer evidence.

Last reviewed: 2026-08-21

## Active Charter

HEAD Agent Core is a provider-neutral, authority-preserving execution-lineage plugin for Codex, OpenCode, and future runtimes. It connects `Whole-plan HEAD`, risk-proportional Observe/Session/Run/Authority lanes, deterministic Context Capsules, explicit ExecutionContracts, bounded Executors, evidence-linked ResultPackets, logical Fresh HEAD review, ReviewDecisions, and reproducible Execution Lineage without making Git, GraphDB, provider sessions, or a native worker prerequisites for meaning or recovery.

The active proof order is: finish one connected provider-neutral implementation vertical; retain only the minimum fail-closed authority, credential, identity, scope, and process checks while building it; then concentrate validation in integrated cross-runtime, recovery, installation, and GraphDB E2E milestones. The current user-authorized remote target is `neopick`. A remote graph remains a rebuildable derived projection, inferred product concepts remain candidates until ReviewDecision, and database reset authority is limited to a proven incompatible reserved HEAD Agent schema in the exact selected database.

At every material checkpoint, re-read this charter, the universal invariants, the current milestone, and the contracts for the subsystem being changed. User and system instructions plus approved project Canon outrank this document.
...
After one inbound author JSON has been GET-verified, interpreted, and reported,
move that exact create-only file from `to-friend/` to `processed_bin/` without
overwriting an existing destination. Never move an unverified or unreported
message, and never alter its JSON body.
...
## Product Operating Loop boundary

Product learning extends the Product Graph without becoming the HEAD control plane. Ordinary conversation may keep `observed-fact`, `hypothesis`, and `inferred-meaning` as explicit non-persisted notes. When recovery or audit requires artifacts, the connected flow is `ProductSignal(observed-fact) -> ProductHypothesis(hypothesis) -> ProductInitiativeCandidate(inferred-meaning) -> explicit user ReviewDecision -> ReviewedProductInitiative(approved-decision)`. A candidate may instead carry explicit inline reasoning without persistent Signal/Hypothesis prerequisites. A reviewed Initiative links to an exact current Feature, a separate non-canonical ProductFeatureCandidate, or an explicit mapping gap; the default path resolves that link once at accept review. It does not mutate Product Canon.

Execution remains a separate authority axis. `ChangeSet -> accepted ResultPacket and execution ReviewDecision` may support an `OutcomeObservation` whose epistemic class is only `observed-fact` or `derived-projection`. The observation cannot declare product success, change Feature status, or promote meaning. HEAD reevaluates the whole outcome using this evidence.

The product loop and continuity view obey these fixed boundaries:

1. five epistemic classes stay explicit: observed fact, approved decision, inferred meaning, hypothesis, and derived projection;
2. Signal-to-Initiative reasoning is not one automatic promotion chain;
3. Product Graph nodes do not own orchestration, tool routing, core context selection, or ReviewDecision;
4. GraphDB is an optional materialization of the same local derived graph;
5. `HEADContinuitySnapshot` is generated on demand from exact current identities, is never persisted, never rewrites the objective, and never replaces Session/Run checkpoint recovery;
6. Fresh HEAD review is a review instrument, not a replacement for continuous whole-outcome HEAD ownership.
...
The plugin must not inherit the reference runtime's OpenCode-only, Herdr-only, `.claude`-path, POSIX-service, shell-wrapper, tab/pane, or provider-session coupling as core identity. When a desired behavior is currently available only through a Herdr-specific mechanism, preserve the behavior by translating it into a provider-neutral contract, injected host evidence, or a separately owned optional adapter instead of adding Herdr binary, socket, command, pane, or TUI knowledge to this plugin. The functional outcome remains a goal; vendor coupling does not. A host-specific adapter remains optional and cannot define HEAD Session, Run, project, or semantic graph identity.
...
| --- | --- | --- |
| initialization and resumable phases | installed `init`/`resume` composes project verification, managed release-path convergence, onboarding start/recovery, and same-identity repeat/resume behavior | retain this single public path and its transaction boundaries |
| exact project/caller fencing | project, Session, authorization, lease, operational-root, live Session, and consequential live Run fences pass for Codex and OpenCode | preserve the common fence for future runtime adapters |
| launch/wait/result/delivery/cleanup | Codex and OpenCode use one native descendant-tree supervisor, transcript-free result draft, canonical ResultPacket application, and Fresh HEAD review path | retain semantic parity and owned cleanup for every supported runtime |
| canon/checkpoint recovery | a fresh process recovers completed provider-neutral records and replaces Codex with OpenCode while preserving canonical Project and HEAD Session identities | keep provider state operational and optional rather than recovery authority |
| durable role coordination | host-issued bindings, authority generation, durable inbox/read/bounded-wait-reply/idempotency state, four role-bound MCP operations, append-only endpoint targets, and delivery-separated receipts are implemented host-locally | retain the non-authoritative Core contract; preserve already-running exact endpoint replacement and worker-question/HEAD-reply evidence through the generic host-driver boundary without importing host-specific identity into Core |
| immutable release and rollback | user-scoped releases, verified native artifacts, and the generated Codex marketplace snapshot are content-identified; activation and rollback use recoverable pointers | retain byte-stable cross-platform packaging and make future catalogs projections of the same release |
...
| durable role messaging | adopt as an optional host-local provider-neutral Core after the completed Session/Run vertical; live host delivery remains generic driver or external-adapter work | messaging stays project/HEAD-Session/generation scoped, sender role is host-bound rather than self-claimed, and no message creates a new authority identity |
...
11. Re-index and verify the resulting Product Canon projection in the temporal GraphSnapshot; expose onboarding status and evidence through CLI and typed MCP, while keeping status inspection read-only and candidate promotion limited to the explicit onboarding review transaction.
...
Current v0.5 alpha foundation, verified through 2026-08-19 and retained by later milestones:
...
- CLI exposes onboarding start/status/review/artifact commands; the guided Skill uses typed MCP for read-only status, idempotent initialize/resume, and explicit user-authored onboarding review through the same Core identities; dependency-free verifiers cover conversation-native readiness plus existing/new/pre-canon/empty/revise/reject/selection/projection/traversal-exclusion/tamper/secret/migration/Git-absent/GraphDB-absent/Go-binary-absent scenarios in CI;
...
- explicit CLI or typed `head_markdown_projection_build` generation and read-only CLI/MCP status expose graph/source freshness and published-view drift without making document generation a World Model indexing prerequisite; modified generated pages are never overwritten and may be captured as immutable, content-derived `DocumentChangeCandidateSet` evidence with no instruction or promotion authority;
...
- CLI can set/read post-refresh policy and read outcomes; MCP remains read-only for that policy and refresh trigger, while the separate typed Markdown build operation may explicitly regenerate only through the existing deterministic document adapter; Git and GraphDB are not consulted, post-refresh artifacts are excluded from Context Capsules, and legacy refresh artifacts remain verifiable;
...
| HF-008 semantic restore plus optional conversation continuity | P2 restore completes first; an actual already-running Codex HEAD exact attachment preserves the checkpoint/projection and `.head` bytes, while missing/stale attachment falls back to a disclosed fresh logical HEAD without provider identity | **verified** |
| HF-009 independently ownable worker and HF-010 integration | an actual Codex one-shot is owned by one P3 dispatch, reports P5 pending/completed wait, rejects duplicate authorization consumption, creates a P3 ResultPacket, requires Fresh HEAD accept, and integrates one explicit P2 checkpoint | **verified** |
| honest Host/platform portability | Windows actual live evidence uses native Job Object ownership; portable PlatformAdapter contracts and POSIX process-group fixtures cover other hosts without claiming unrun three-OS live parity | **verified** |
| live role coordination across workspace hosts | durable role state, exact endpoint targets, per-process proof plus current-binding ownership, binding-scoped offline endpoint resolution, a production host-export filesystem bridge, and actual OpenCode send → claimed host request → owned Codex start → exact wake ack, followed independently by Codex read/reply, pass without Herdr or project mutation | **verified** |
...
| remote graph acceptance | fresh NeoPick evidence covers 9/9 compatible reserved types with 46 unrelated types preserved; 8,037 nodes/12,441 edges; zero-batch no-change receipt; semantic-equivalent prepared reads; isolated initial/delta/checkpoint interruption-resume/pointer-conflict-recovery; credential non-persistence; and exact acceptance-namespace cleanup to zero records | **verified** |
| installation, upgrade, distribution, and rollback | content-addressed user install/upgrade/rollback/remove, five-platform CI builds, exact-version native package verification, and a content-verified Git-backed Codex marketplace snapshot/install path pass; OpenAI universal-directory review is a separate optional publisher action | **verified** |
...
- 2026-08-21: the original author directly opened pushed commit `324947e`, its authority contract/design and focused tests, and compared them to the original `TECHNICAL_ARCHITECTURE.md`, `HEAD_CORE.md`, Product Graph `FOUNDATION.md`, and `CORE_SPEC_V0.md`. The verdict was `original philosophy alignment: yes`, `graph-record boundary alignment: yes`, all four requested sub-boundaries `yes`, `blocking correction: none`, and `philosophical superiority: yes` under the fair criterion that the provider-neutral implementation makes the original implicit boundaries explicit, executable, falsifiable, and portable without losing original strengths. The author did not run npm tests; local evidence remains 57 pass, zero fail, and one expected native-fixture skip. Two non-blocking counterexamples are incorporated immediately: candidate versus reviewed Feature names receive different planes, and P3/P4/P5 cannot amplify into P2 checkpoint recovery. This author reply is advisory evidence, not a ReviewDecision.
...
- 2026-08-19: completed the explicitly authorized live `neopick` graph acceptance without resetting the database. Compatibility preserved 46 unrelated schema types; activation reached `verified-active-current` for GraphSnapshot `graph-snapshot-98a6a3779cd0fa43ba64c9aa` with 8,037 nodes and 12,441 edges, and both exact and neighborhood server traversal conformed. The remote graph remains a rebuildable derived projection, credentials were not persisted, and the 23 inferred onboarding concepts remain evidence-linked candidates rather than Product Canon.
- 2026-08-19: completed the first user-scoped immutable distribution lifecycle. Matching plugin/package metadata and every included regular file produce one content-derived release identity; symlinked inputs, path escape, integrity drift, version mismatch, unsafe install roots, and unowned command collisions fail closed. Install/upgrade stages and verifies a new release before switching the recoverable active pointer, rollback selects the prior verified release, normal uninstall preserves releases, purge requires an ownership marker, and neither removal path traverses project `.head`, Git, or GraphDB state. An isolated Windows user-home E2E proves the global launcher, rejected-upgrade preservation, distinct upgrade, rollback, and removal. Codex marketplace publication and automatic native artifact download remain planned adapter work.
- 2026-08-19: completed the public initialize/resume composition and installation-projection recovery slice. Source or installed `init` now creates one project/HEAD Session and immediately starts onboarding; repeat `init` or `resume` returns existing pending-review/ready authority without rewriting the state pointer, while `awaiting-evidence` can deterministically resume after new source evidence appears. Install/upgrade may invoke the same path, and release changes rewrite only digest-verified managed OpenCode projections; runtime-set drift, modified managed files, and unowned paths fail closed without overwrite. The isolated user-home E2E proves install, Git/GraphDB-free onboarding, non-duplication, evidence resumption, drift refusal, upgrade convergence, rollback convergence, and project preservation. The rule pass also removed a duplicate native-supervisor deferral and replaced MCP's stale hard-coded version with package metadata. OpenCode actual-provider conformance, provider-loss/replacement E2E, marketplace/native delivery, newcomer walkthrough, and the final rule audit remained open at this checkpoint.
- 2026-08-19: bound optional exact `provider/model` selection into ExecutionAuthorization `0.3.0` while retaining verified `0.2.0` reads. Codex and OpenCode now receive the selected model only from the immutable authorization; changing it changes the authorization identity, and credentials/endpoints remain operational rather than semantic artifacts. A fresh Node process recovered a completed Codex invocation from project artifacts, regenerated capability evidence, preserved the same canonical Project/HEAD Session, and completed an OpenCode fixture invocation without Git, GraphDB, or provider-session identity. The live OpenCode custom-provider path still emitted a generic provider error even though a direct compatible request to the same endpoint/model succeeded, so no actual OpenCode conformance is claimed. Per the user's correction, a separately observed Bun crash was caused by external security software and is excluded from all model-selection, authentication, and adapter-correctness conclusions.
...
- 2026-08-20: completed the first conversation-native onboarding implementation slice. The bundled `head-agent-onboarding` Skill now detects uninitialized, review, drift, graph, document, and ready states; typed MCP operations call the existing idempotent initialize/resume, explicit onboarding ReviewDecision, and deterministic Markdown projection Core paths; and the CLI remains the equivalent recovery/automation surface. An isolated walkthrough preserves Project and HEAD Session identities, requires explicit review before Product Canon, verifies World Model/graph/Context/documents, works without Git or GraphDB, and persists no credential values. Both Skill validators, the plugin manifest validator, the 34-test integrated suite, onboarding verifier, runtime adapter verifier, and newcomer installation lifecycle pass. A fresh NeoPick prepared-traversal attempt correctly fell back because this active process lacks the stored credential reference values; local activation and topology receipts remain current and recoverable, but this run does not claim a new live remote acceptance or perform a remote write.
- 2026-08-20: implemented the first incremental ArcadeDB write data plane without changing graph or Product Canon authority. Activation derives immutable sync manifests from the exact base and target GraphSnapshots; compact carry/rebase batches avoid retransmitting snapshot-only node changes and stable relations, bounded full-record batches cover true additions/changes, and each batch is re-read before an immutable remote checkpoint is accepted. Interrupted uncheckpointed work is idempotently replayed, verified checkpoints resume, transient transport work has bounded retry, the complete topology reconstructs the canonical GraphSnapshot through a small topology-backed envelope, and baseline plus prepared-traversal conformance run against a staged pointer. Only then can exact-predecessor CAS advance the remote pointer and the verified local recovery mirror, after which a content-addressed sync receipt is persisted. Integrated evidence covers first upload, zero-batch no-change, small delta/rebase, checkpoint interruption/resume, pointer conflict, prepared traversal, corruption rejection, and local recovery. The reserved schema grows compatibly from seven to nine types. No fresh live NeoPick write is claimed because the active process still lacks both credential reference values; the prior 8,037-node/12,441-edge live acceptance remains historical evidence rather than proof of this new path.
- 2026-08-20: removed the measured duplicate prepared-traversal pointer round trip without caching remote authority or changing graph/query/result identity. The same adapter's initial verified pointer read now carries a non-serializable, one-shot, instance-bound token into the immediate synchronous prepared query; invalid, foreign, missing, or consumed tokens perform the ordinary remote read. Internal response and receipt construction reuse the already-verified request object, while construction, adapter entry, and client receipt verification remain independent trust boundaries. The reviewed seven-query fixture retains identical semantic and deterministic-cost identities, zero writes, and zero full snapshot/topology reads while pointer requests fall from the previous 14-call shape to seven and normalized pointer-response bytes from 10,052 to 5,026. No live latency improvement is claimed until credential references permit a fresh NeoPick run.
- 2026-08-20: added a generated Git-backed Codex marketplace distribution without changing the source repository into a catalog-shaped monorepo. The builder reuses the immutable distribution allowlist and file-digest manifest, emits exactly one local-path catalog entry plus a content-derived source-commit marker, excludes development/test/cache/build trees, and independently rejects catalog path escape, unexpected tree content, symlinks, manifest drift, or source-repository mismatch. CI verifies packaging with the integrated suite, waits for all five native builds, validates any existing target branch before replacement, and publishes only through an exact expected-commit force-with-lease. A real local Codex marketplace add/install listed `head-agent-core@head-agent-plugin` at `0.3.0-alpha.56`; the installed cache passed the official plugin validator, after which the plugin, marketplace registration, cache, and temporary snapshot were removed. This provides an installable Git marketplace while keeping OpenAI universal-directory submission and publisher identity/review as a separate user-owned action.
- 2026-08-20: completed the next prepared-traversal client verification optimization without weakening the construction, adapter-entry, or client-receipt trust boundaries. The client now independently verifies the returned receipt digest and binds it to the untouched same-stack locally built request, but no longer re-parses and re-hashes that entire already-verified request or self-verifies a locally constructed expected receipt. A distinct valid request receipt replay is rejected. The seven-query fixture retains the exact graph, request, result, and deterministic cost identities, performs seven receipt-digest and seven request-binding checks, records zero full request reverifications at receipt, and preserves one pointer/manifest/traversal read per query with zero writes or full graph/topology reads. No live NeoPick latency improvement is claimed until both credential references are available to a fresh process.
- 2026-08-20: closed the remaining cross-platform source-checkout byte drift in the installation plane. The source repository now normalizes detected text to LF on checkout while preserving binary detection, and the generated marketplace branch continues to freeze the assembled bytes with its stricter `* -text` policy. This makes fresh Windows, macOS, and Linux clones feed the same committed text bytes into the content-addressed distribution instead of allowing host `core.autocrlf` to change release identity. Workflow triggers include the source attribute policy so future changes cannot bypass distribution verification.
- 2026-08-20: reconciled the progressive status sections with the current completion audit. The reference-adoption table now records the already verified Codex/OpenCode common supervisor, result application, Fresh HEAD review, fresh-process provider replacement, native artifact delivery, and generated marketplace path; historical v0.5/v0.6 sections are labeled as retained foundations rather than active milestones; and the deferred list no longer misclassifies the implemented resumable ArcadeDB delta and pointer compare-and-swap path. This changes no runtime behavior or authority boundary.
- 2026-08-20: completed the conversation-native optional GraphDB mutation path without making remote storage a prerequisite or secret store. Typed MCP operations now expose database initialization and projection activation through the existing Core transactions, accept no credential values, require explicit confirmation for initialization and a separate confirmation for remote projection writes, and retain the exact-target reset fence for a proven incompatible HEAD-reserved schema. Read-only status tools are marked as open-world observations, the onboarding Skill stops on unavailable environment references instead of treating local fallback as remote success, and the CLI remains the equivalent recovery/automation path. Mock-transport MCP E2E covers confirmation rejection, compatible database reuse, verified incremental activation, and credential-free results; fresh NeoPick acceptance still requires the configured references in the active host process.
- 2026-08-20: added a non-network GraphDB connection preflight after the user correctly questioned why credentials written in goal text were not sufficient. The exact-child bridge now reports only whether the two configured environment reference names are present; it returns no credential, endpoint, or database values and performs no request. `head_graphdb_connection_preflight` exposes that bounded evidence to the onboarding Skill, which explains that chat or goal text is not runtime secret injection and stops before compatibility or mutation until both references are inherited by the plugin host. A tracked conversational E2E proves the missing-reference result and zero target-value disclosure.
- 2026-08-20: completed fresh NeoPick incremental GraphDB acceptance after both environment references became visible to the plugin process. The live database reached all nine compatible reserved types while preserving 46 unrelated types. The 8,037-node/12,441-edge projection produced a zero-batch no-change receipt with unchanged predecessor/target pointer and verified snapshot, topology, local mirror, and atomic transition; seven read-only prepared traversals preserved semantic identity, used no full snapshot/topology reads or writes, and measured 2,820.4 ms end-to-end median with 137.7 ms database expansion. An isolated live project namespace then passed initial upload, four-batch small delta, interruption after one checkpoint, checkpoint resume, pointer-conflict rejection, and conflict recovery before exact cleanup to zero records. The acceptance exposed and fixed three real transport defects: forced Node bridge exit after a valid large response, non-portable SQLScript CAS, and endpoint-level suppression of distinct parallel semantic edges. Credential values remained unpersisted and the production project/Product Canon were not mutated.
- 2026-08-20: completed the minimal Product Operating Loop and portable derived HEAD continuity slice after a file-mail design review with the HEAD Agent author. Immutable observed Signals, explicit Hypotheses, inferred Initiative candidates, user-scoped Initiative ReviewDecisions, separate reviewed Initiatives, exact/candidate/gap Feature resolution, and execution-bound OutcomeObservations now project into the local World Model without changing Product Canon or declaring product success. Feature candidates are seed-bound, Initiative candidates are created once with exact resolution identities, and review is byte-invariant over the candidate. `HEADContinuitySnapshot` is generated only on demand, persists nothing, carries no instruction, promotion, recovery, or objective-rewrite authority, and leaves Session/Run checkpoints canonical. The author re-review confirmed the former immutability blocker was resolved and reported no remaining blocking correction; no GraphDB activation, Release artifact, remote write, commit, or push was part of this slice.
- 2026-08-20: applied the original HEAD Core's proportional-machinery correction to the universal plugin surface. Epistemic ProductLearningNotes are non-persisted by default; a pure Observe/Session/Run/Authority recommendation selects the lightest safe contract without granting authority; repeated same-process reads may reuse disclosed write-invalidated verified snapshots while `--fresh` restores full verification; and Initiative proposals may use inline reasoning and defer Feature resolution until a separate acceptance ReviewDecision. Durable Product Canon, candidate immutability, user-owned decisions, external writes, credentials, recovery canon, and independent trust boundaries remain fail-closed. The original/current structural comparison and bounded superiority proposition are documented for a new author review; this entry does not claim that review has passed.
- 2026-08-20: after the author found no blocking correction but withheld comparative-superiority approval, narrowed the visible CLI default to note, optional advisory lane, status, and authority-bound Initiative review while retaining the complete surface under `help-all`. Added direct regression evidence that lane recommendation is not required and that a cached status read cannot replace candidate verification at review. This removes two remaining mental-load and trust-boundary ambiguities; it does not claim OpenCode/Herdr-native coordination completeness or overall superiority.
- 2026-08-20: completed the bilateral original/current implementation comparison requested by the HEAD Core author. A fresh-process fixture verifier proved Codex-to-OpenCode adapter/supervisor replacement preserves exact Project and HEAD Session identities, creates a new authorization, recovers from project artifacts, persists no provider-session identity, and requires neither Git nor GraphDB. The original base inventory and architecture were contrasted with the plugin's Git-independent ChangeSet and temporal tests. The author's final file-mail verdict confirmed provider coverage `yes`, provider replacement `partial` within that fixture receipt, integrated product/change/temporal release breadth `yes`, distribution breadth `yes`, and accepted this as a scope-limited comparative advantage for universal provider-neutral plugin use. Overall or philosophical superiority, Herdr-native completeness, and live provider-network replacement completeness remain explicitly unclaimed; the original remains stronger in OpenCode/Herdr live coordination.
- 2026-08-20: implemented the original author's compaction continuity correction as an explicit provider-neutral recovery vertical. A content-addressed Session/Run checkpoint, not a new compaction canon, now preserves purpose, approved decisions, current position, next expected result, open reviews, and verified active-Run lineage. One bounded CompactionEpoch binds a non-persisted raw continuation token by hash; provider success, canonical checkpoint verification, at-most-once consumption, newer-real-user-turn supersession, explicit abort, and derived non-authoritative RecoveryReceipts are available through advanced CLI/MCP/Skill surfaces. Provider summaries, transcripts, session identities, ContextCapsules, and HEADContinuitySnapshot cannot rewrite recovery direction; Core does not invoke provider compaction, approve reviews, change Product Canon/candidates, or require Git/GraphDB.
- 2026-08-20: the original author returned `blocking correction 없음` for the compaction slice and confirmed that the supplied checkpoint/epoch/token/supersession/provider-boundary contract satisfies the requested eight invariants if the implementation and tests are as reported. This verdict is evidence-limited: the author did not directly open commit `f5fa9ef`, so it records no independent source audit and makes no claim of philosophical superiority, overall advantage, or complete live provider compaction. Local integrated regression, plugin validation, and Skill validation remain the direct implementation evidence.
- 2026-08-20: after sending the original author a nine-part role-coordination design request and receiving no reply during the verified 15-minute polling window, implemented only the invariants shared by the original source and the current authority model, leaving the contract explicitly open to author correction. Role identity is derived from a one-time host-issued binding rather than message arguments; Project, canonical HEAD Session, current host-local generation, and binding replacement fence every operation. Durable send/read/immutable reply/idempotency state lives outside the project, message authority flags are all false, and optional delivery occurs only after durable acceptance with no automatic ambiguous retry. CLI administration is advanced-only and the role-bound MCP surface contains exactly send/read/reply. Local integrated regression, fresh-process recovery, newcomer installation, distribution rollback, plugin validation, and Skill validation pass. This narrows but does not claim replacement of the original's exact live Herdr caller/target advantage; an actual multi-role live-host E2E and direct author source audit remain gates.
- 2026-08-20: sent direct source-audit request `20260820T102630Z--role-coordination-source-audit` for pushed commit `f0af147` and later design proposal `20260820T104722Z--live-workspacehost-herdr-design`; both create-only writes and GET verification passed, while the bounded 15-minute audit and five-minute design-response windows produced no author reply. Silence was not treated as approval. Direct comparison with the original `HostAgentCommService` bounded the independent implementation around a verified active `WorkspaceHostAdapter`, host-injected rather than tool-supplied caller evidence, append-only hash/sequence/previous endpoint target chains, current binding/target and project-CWD fences, fresh pre/post snapshots, exact message/endpoint acknowledgments, target TOCTOU detection, and non-retried ambiguity. A proposed Herdr binary/socket/CLI driver was then withdrawn on user direction: Herdr-specific functionality must be achieved through the provider-neutral injected driver contract or a separately owned optional adapter, not embedded into this plugin. Deterministic two-fresh-process Codex/OpenCode generic-host delivery, stale/replaced/detached/rolled-back targets, partial send, topology change, cross-project CWD, provider-session absence, and zero `.head` mutation remain the Core evidence. Actual host-specific live E2E and an author source verdict remain gates before comparative superiority.
- 2026-08-20: implemented the Herdr-independent production `host-export` bridge rather than narrowing the live-coordination outcome. A trusted host publishes content-addressed endpoint snapshots outside the project, starts each role-bound MCP process with an exact host project/caller tuple, acquires a create-only pre-effect claim for a create-only filesystem delivery request, and returns a content-bound create-only acknowledgment. A claim that survives without acknowledgment is ambiguous and cannot be automatically consumed again. The Core still performs current binding/target, project-CWD, fresh pre/post snapshot, exact endpoint/ack, target TOCTOU, and non-retried ambiguity checks. Integrated evidence uses two fresh role MCP processes and a separate live host consumer, rejects project/export overlap, host-project mismatch, missing acknowledgments, pointer tamper, claim replay, and provider-session identity disclosure, and leaves `.head` byte-identical. This replaces the withdrawn Herdr-specific route with a provider-neutral production bridge, but actual Codex/OpenCode provider-client wake/tool consumption and direct author source audit remain open before comparative superiority.
- 2026-08-20: recovered a valid late original-author reply that the courier had incorrectly missed because it marked filenames seen before successful JSON validation and separately filtered out a valid `in_reply_to: null`. The author found one blocking correction in commit `3443f69`: snapshot membership alone did not prove the current process owned the claimed endpoint, and another role could copy its tuple unless attach stayed host-composition-only with unique ownership. The first host-export commit `2e9f577` supplied a real external request/claim/ack bridge but still relied on injected tuple values for caller identity. The follow-up therefore binds every exported endpoint uniquely to the exact current coordination `bindingId` and a host-issued per-process proof hash, injects the raw proof only into that MCP process, revalidates both on every fresh snapshot, and rejects copied tuples, foreign bindings, forged proofs, and duplicate endpoint/terminal/binding/proof ownership. No raw proof enters project state, receipts, or messages. This closes the reported caller-fencing mechanism locally; it does not count as author approval until the author directly audits the corrected commit.
- 2026-08-20: the original author directly re-audited production commit `309947b` and judged the previous live-caller blocker resolved with `blocking correction 없음`. The author confirmed that process proof plus current-binding ownership preserves the original live-caller invariant in a provider-neutral form, while requiring the in-memory fixture to remain explicitly test-only and the production MCP to have no proof-free fallback. Added counterexamples cover missing-proof production startup, copied tuple plus valid foreign proof, duplicate ownership, generation rotation rejecting old binding/proof, and raw-proof non-persistence. The author still prohibits a live-replacement claim before actual Codex/OpenCode wake and send/read/reply E2E; this verdict therefore closes this blocker but is not one of the three unconditional final superiority verdicts.
- 2026-08-20: completed the first Herdr-independent actual provider-client offline wake and role round trip. A real OpenCode 1.17.18 client called `head_coordination_send_message`; because a never-attached recipient could not previously produce a host request, the production adapter now treats the trusted snapshot as authority-free offline reachability, resolves only the exact current recipient binding, and binds that identity into request/claim/ack verification. The host acquired the create-only claim before starting a new Codex 0.144.3 HEAD process, whose distinct raw proof and current binding were revalidated before its actual `read-inbox` and immutable `reply` calls. Windows Job Objects proved descendant ownership and cleanup for both clients, `.head` remained byte-identical, actual provider thread/session references were hashed only in the ephemeral verifier and absent from scoped state, and raw proofs/binding tokens were absent from all scoped files. Explicit detach still suppresses offline wake, and copied/foreign/stale binding or proof states remain fail closed. The first verifier incorrectly delayed its exact acknowledgment until durable reply and was therefore not final source-audit evidence.
- 2026-08-20: the original author reviewed the offline-registration design and accepted snapshot-only unique current-binding resolution, sender/recipient proof separation, proof creation before publication, claim-before-start, and proof non-reuse as the correct provider-neutral extension. The author also distinguished this wake-mail extension from the original already-live pane target and issued one blocking correction: delivery acknowledgment must report the applied wake, never recipient behavior. The verifier now waits only for the claimed Codex provider process start under owned native supervision, proves the reply is still pending, writes the exact ack, and then independently verifies Codex read/reply plus OpenCode completion. This removes reply coupling without weakening the actual client, process-proof, durable-message, or cleanup evidence. The corrected commit still requires direct author source audit and does not yet carry any final comparative-superiority verdict.
- 2026-08-21: the actual original author directly re-audited pushed commit `cbc49bc` and reported no blocking correction, while withholding all three final superiority verdicts for three precise evidence gaps: actual already-running Codex/OpenCode exact attachment plus endpoint replacement, bounded one-shot interrupt/close, and a worker authority-question round trip that waits for HEAD without treating its answer as a ReviewDecision. The provider-neutral correction adds bounded inbox/reply waits that reauthenticate on every poll; reply observation is delivery-independent and explicitly non-authoritative. At pushed source commit `ef59d21`, the production opt-in verifier passed actual Codex 0.144.3 (`gpt-5.6-sol`) and OpenCode 1.17.18 (`opencode/big-pickle`) clients: both provider children started before the delivery request, current endpoint replacement was exact, stale endpoint requests were zero, claim caused no spawn, OpenCode called send plus bounded wait-reply, Codex called bounded read-inbox plus immutable reply, and no ReviewDecision authority appeared. Separate actual Codex interrupt and OpenCode close clients ended under Windows Job Object ownership with verified tree cleanup; resume/stream stayed false. `.head` remained byte-identical and no raw proof, binding/control token, or provider-session identity persisted. The renewed direct author verdict remains pending.
- 2026-08-21: formalized five executable semantic planes and the Graph/record boundary. P1 owns Product Canon and explicit ReviewDecision, P2 owns provider-independent Project/Session/Run recovery lineage, P3 owns results/candidates/receipts, P4 owns rebuildable graph and continuity views, and P5 owns host-local operational effects; Distribution and Host remain architectural planes outside that lattice. The original author directly opened pushed commit `324947e`, compared its source and tests with the original architecture, reported original-philosophy alignment `yes`, Graph/record alignment `yes`, `blocking correction: none`, and `philosophical superiority: yes`. Follow-up commit `a1e32f7` then distinguished reviewed Canon Features from Feature candidates and rejected generic P3/P4/P5-to-P2 amplification while preserving legacy boundary reads; focused author confirmation remains pending.
- 2026-08-21: completed local validation of the artifact-only Session restore and reviewed-result checkpoint-integration slice pending direct author source audit. SessionRunCheckpoint protocol 0.3 freezes an immutable Session pointer and optional exact accepted-Run integration reference. `session-restore` returns one deterministic non-persisted P4 projection from the current checkpoint and verified P2 lineage, ignores provider-session environment identity, keeps resume/stream disabled, rejects pointer drift, and preserves exact direction when P3 ResultPacket or integration-request evidence is missing without fabricating Fresh HEAD review. `run-integrate-checkpoint` requires the current reviewed Run plus exact `accept` ReviewDecision and explicit HEAD/user recovery fields, freezes them in a create-only P3 request before the checkpoint write, binds that request identity and input hash into self-contained P2 fields so the lower-level API cannot bypass or diverge from the transaction, converges identical concurrent attempts on the reviewed-time-derived identity, rejects divergent replay and every non-accept disposition, and records only a P3 receipt. The old time-based direct checkpoint format is retired; the public checkpoint command and migrated product tests write only the content-addressed P2 format, while 0.1/0.2 artifacts remain audit-readable but cannot drive current restore. Full regression passed 65 with zero failure and one expected native-fixture skip; Plugin and both Skill validators, distribution lifecycle, Codex marketplace packaging, fresh-process Codex-to-OpenCode replacement recovery, newcomer, and conversational onboarding verifiers passed. No Herdr-specific behavior or provider resume was introduced. The following direct author audit corrected the original HF-007/008/009/010 mapping before any comparative completion claim.
- 2026-08-21: received direct original-author audit of pushed commit `2a0d77c`. The author confirmed exact source inspection, original-philosophy alignment `yes`, P1/P2/P3/P4 and Graph/record alignment `yes`, `blocking correction: none`, and no philosophical-superiority regression. The author correctly narrowed the numbered comparison: original HF-007 is compaction, HF-008 is OpenCode Session restore, HF-009 is independently ownable worker dispatch, and HF-010 is completed worker-branch integration. Therefore artifact-only semantic restore remains intentionally partial relative to provider `ses_*` resume, reviewed-result integration belongs with HF-010 rather than HF-009 dispatch, and compaction/provider-loss continuity must not be mislabeled HF-010. A new hostless verifier now uses independent process A integration and process B restore consumers, proves equal projection and direction under fake Codex/OpenCode session identities, recovers request-before-checkpoint and checkpoint-before-receipt crash shapes without duplicate checkpoints, preserves P2 direction while disclosing deleted P3 evidence, converges concurrent identical integration to one checkpoint, rejects divergent and non-accept attempts, and executes one read-only resident-HEAD next move without treating injected inbox text as direction. Git, GraphDB, WorkspaceHost, Herdr, and provider resume are not required. Full regression passed 66 with zero failure and one expected native-fixture skip; Plugin and both Skill validators, distribution install/upgrade/rollback with the verifier packaged, Codex marketplace packaging, newcomer installation, and conversational onboarding passed. Final overall completeness and overall superiority verdicts remain pending direct audit of the next pushed commit.
- 2026-08-21: implemented and locally validated the original author's exact portable A/B/C reconsideration slice. Authority contract `0.4.0` places `ContinuationOutcome` and bounded wait in P5 and `BoundedWorkerDispatch` in P3. Actual Codex one-shot evidence closes dispatch/own/wait/ResultPacket/Fresh HEAD accept/checkpoint integration with duplicate authorization rejection. Actual already-running Codex/OpenCode coordination proves P2-first exact attachment, unchanged recovery direction and `.head` bytes, endpoint replacement, non-authoritative messaging, and owned cleanup on Windows; portable PlatformAdapter contracts and POSIX fixtures remain honest non-live evidence for other hosts. Full regression passed 67 with zero failure and one expected native-fixture skip; Plugin and both Skill validators, distribution lifecycle, Codex marketplace alpha.79 packaging, fresh-process provider replacement, newcomer, and conversational onboarding passed. This is local implementation evidence only: exact commit/push and direct original-author source audit remain required before any final superiority claim.
- 2026-08-21: the original author directly audited exact pushed source `fd9ad3baf8879bd4b92f18e1b9cf4562358d737e`, confirmed original-philosophy alignment, Graph/P1-P5 alignment, and preferred A/B/C implementation with `blocking correction: none`. An initial conclusion incorrectly reused OpenCode resume/stream and installed Herdr/live-pane parity after the author's own governing criteria had classified them as optional or excluded. In criterion-resolution reply `20260820T183022Z--completeness-criterion-resolution`, the author explicitly corrected that inconsistency: P2 restore closes semantic HF-008, attachment remains optional P5 continuity, resume/stream remain optional adapter capabilities, one-OS live plus portable Host adapter evidence satisfies the gate, Herdr-specific implementation is not required, and no user-consumer outcome remains. The final unconditional comparative verdict is `blocking correction: none`, `philosophical superiority: yes`, `overall completeness superiority: yes`, and `overall superiority: yes`. This external audit is comparative evidence, not a ReviewDecision and not project authority.
- 2026-08-21: fast-forwarding the audited vertical to `main` exposed a stale prepared-traversal fixture identity in Linux CI before any marketplace publication. Recomputing the fixed 64-file graph under temporal protocol `0.9.0` preserved 274 nodes, 480 edges, the 20,478-byte prepared query, zero query writes, and the same 9,754-basis-point reduction; the explicit P1-P5 authority boundary and empty Product Operating summary increased only the conservative full-reload baseline from 833,590 to 834,638 bytes, so saved bytes increased to 814,160. The reviewed expectation and current Graph projection documentation now bind the new graph/request/result/cost identities. This correction changes no Product Canon, recovery direction, provider contract, or author verdict; marketplace publication remains gated on the repaired benchmark plus the complete CI build.
- 2026-08-21: corrected the development/runtime direction boundary before implementing the next performance slice. This file is now explicitly a repository-development goal and append-only work history, is excluded from installed releases, and cannot serve as Product Canon, target-project direction, Context input, WholePlan authority, or recovery authority. Generated project instructions and the bundled Skill now derive direction only from the active user request, verified project Canon, current Session/Run recovery state, explicit ReviewDecisions, and the stable runtime architecture/authority contracts. The named large-project/GraphDB acceptance target and its review proposal remain historical development fixtures only; a new distribution gate rejects their identity or this development document from runtime paths or text. `performance-fast-path-design.md` confines transport, read reuse, single-pass scanning, and immutable descriptor caching to P5 while retaining byte hashing, the JavaScript semantic oracle, complete Context selection, fail-closed integrity, and exact identity/error conformance. Full local regression passed 67 with one expected native-fixture skip and zero failures; distribution install/upgrade/rollback plus intentional leak rejection, Plugin validation, and Skill validation passed. A create-only file-mail request asks the original author for counterexamples and a blocking-correction verdict; no reply is treated as approval.
