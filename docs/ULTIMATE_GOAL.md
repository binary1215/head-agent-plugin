...
Immediate next slice: the conversation-native onboarding vertical, typed GraphDB compatibility/initialization/activation path, first verified incremental GraphDB write path, one-read prepared traversal, and generated Git-backed Codex marketplace distribution are implemented. The marketplace snapshot reuses the immutable distribution allowlist and digest manifest, is published only after integrated tests plus all five native builds, and refuses to replace a branch that is not already a verified generated snapshot. OpenAI universal directory submission remains a separate publisher-owned review, not an engineering completion proxy. The immediate GraphDB acceptance task is to re-run the authorized NeoPick no-change, small-delta, interruption/recovery, pointer-conflict, read, and prepared-traversal scenarios when credential references are available to the active process; local integrated evidence is current, but a local activation pointer or fallback result alone is not a fresh live acceptance. After that proof, tune batch size/concurrency and any remaining latency only from measured evidence. OpenCode provider authority remains in each user's resolved global configuration and authentication; this machine and NeoPick are integration fixtures, never product presets or unique authority. Provider-session resume, general stream/interrupt/close, messaging, shell interpretation, TUI scraping, automatic merge, and universal-directory publication remain separate optional work.
...
| remote graph acceptance | local incremental-sync conformance and the earlier user-directed direct NeoPick topology/read acceptance pass; fresh no-change/delta/interruption/conflict acceptance for the new write path still requires credential references in the active process | **partial: fresh incremental-write acceptance pending** |
