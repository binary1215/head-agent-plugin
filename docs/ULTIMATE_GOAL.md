...
- the default Repository World Model path now uses the native-preferred adapter, but the packaged worker intentionally does not advertise `repository.scan.v1`, so scan semantics still execute through JavaScript and no native performance claim is made;
