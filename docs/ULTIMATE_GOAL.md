...
- no live Codex model call or descendant-tree conformance has yet been claimed. An actual invocation is deliberately recorded with `descendantTreeOwnershipValidated: false`, produces failed verification plus an explicit Unknown, and cannot count as an accepted Run result. Cross-platform process-tree supervision, live Session/Run evidence, canonical Run ResultPacket conversion, OpenCode invocation, provider resume/attachment, messaging, and general runtime controls remain deferred.
