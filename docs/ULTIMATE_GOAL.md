...
Immediate next slice: complete the remaining provider-replacement and distribution proof after the public initialize/resume milestone. The installed `init`/`resume` path now composes project verification, drift-free managed integration convergence, and onboarding start/recovery without duplicating Project, HEAD Session, candidate, or Product Canon authority; the same path is available directly from install/upgrade. Next obtain authenticated OpenCode actual-provider evidence through the shared core, prove artifact-only recovery across provider loss/replacement, and decide the Codex marketplace plus automatic verified native-artifact distribution shape before the final newcomer README and rule-deletion audit. The retained NeoPick onboarding Session contains 23 evidence-linked candidates and a bounded `revise` proposal; no Product Canon promotion is implied by remote graph projection. Provider-session resume, general stream/interrupt/close, messaging, shell interpretation, TUI scraping, and automatic merge remain disabled.
...
| Codex and OpenCode share one core identity and contract | common authorization, lease, supervisor, invocation record, result application, and protocol fixtures; Codex live passes, OpenCode actual provider stopped at missing authentication | **partial** |
...
| provider-session loss and context-pollution recovery | canon, Capsules, invocation records, ResultPackets, and Reviews are provider-independent and individually recoverable | **partial: integrated provider-loss/replacement E2E pending** |
