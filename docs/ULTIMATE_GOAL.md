...
Immediate next slice: execute one bounded live Codex Session through the now-supervised `exec` one-shot composition, then execute one consequential live Run through the established `ExecutionAuthorization`, external operational-state boundary, durable at-most-once lease, native descendant-tree ownership, JSONL event normalization, structured result, and ResultPacket draft. The deterministic Run-draft application path is now implemented: only a completed, native-supervised actual-provider Run may create the exact canonical ResultPacket, transition to Fresh HEAD review, and persist an idempotent content-derived application receipt. It is not yet live-conformed. The Session must not manufacture WholePlan/Review artifacts; the Run must return through the complete Execution Lineage and Fresh HEAD boundary. The integrated ArcadeDB activation and prepared-traversal evaluation remains a separate optional `sandbox` E2E that requires explicit risk-informed external-write approval. Provider-session resume, general stream/interrupt/close, messaging, shell interpretation, and TUI scraping remain disabled until actual-provider lifecycle conformance is proved.
...
- structured-result protocol `0.1.0`, lifecycle-receipt `0.4.0`, and ResultPacket-draft `0.4.0` now add one common bounded outcome/evidence/plan-delta/impact/verification/Unknown contract; Session results forbid plan delta and impact radius, raw JSONL remains ephemeral, and immutable transcript-free event/receipt/draft records are recoverable through CLI and read-only MCP;
- a Codex one-shot composition now revalidates the exact authorized executable/protocol/project binding, invokes only the fixed `exec --json --ephemeral` surface with no shell, passes the exact authorized input over stdin, uses a host-local output schema, hashes operational thread references, and persists no prompt, transcript, PID, raw command, absolute project root, or operational path; its protocol fixture is verified for both the lightweight Session result boundary and common lease/cleanup semantics;
- cross-platform process-tree supervision is now implemented as a separate integrity-verified native helper. Windows uses a Job Object with kill-on-close and POSIX uses an isolated process group; operational control files and PIDs stay outside project canon, while durable receipts retain only the helper-manifest digest and bounded ownership/cleanup facts. Windows normal-exit and cancellation fixtures prove removal of a lingering grandchild, and the Codex protocol fixture passes through the same supervisor. A fail-closed, idempotent application bridge now maps only a completed actual-provider Run draft into canonical ResultPacket and Fresh HEAD state. No live Codex model call has yet been claimed, so live Session/Run evidence and accepted live application remain deferred together with OpenCode invocation, provider resume/attachment, messaging, and general runtime controls.
