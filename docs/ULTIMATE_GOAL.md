...
Immediate next slice: improve NeoPick onboarding source scoping and large-project Context Compiler performance before promoting inferred product concepts or publishing the graph to the `neopick` database. The actual Codex Run now conforms end to end: the real provider completed the exact isolated file write, native descendant cleanup, structured draft, canonical ResultPacket application, and Fresh HEAD review without Product Canon mutation or raw transcript persistence. `run-only` is the efficient default and `session-and-run` remains optional broader coverage; neither is a product-level model-call quota. Transport safety remains deliberately generous but finite at 8 MiB total stdout and 2 MiB per JSONL event, independent from the 128 KiB semantic structured-result contract. In the isolated NeoPick projection, 870 eligible files produced 5,299 symbols, 44,476 call observations, and a current 12,643-node/19,907-edge graph, but the 49 inferred candidates mix valid project concepts with `.omo`, bundled model internals, and test doubles. They remain unreviewed and excluded from normal traversal. A realistic Context preview did not complete within roughly three minutes and was cancelled cleanly. Add explicit project source-scope/exclusion evidence, regenerate a reviewable candidate set, and establish a bounded Context latency baseline before any `accept-all`, `neopick` DB initialization, or remote activation. Provider-session resume, general stream/interrupt/close, messaging, shell interpretation, and TUI scraping remain disabled.
