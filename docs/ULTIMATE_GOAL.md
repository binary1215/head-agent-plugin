...
- ArcadeDB projection protocol `0.1.0` now resolves credentials only from environment reference names inside a bounded exact-child HTTP bridge, creates a database-local snapshot/pointer schema, inserts and re-reads immutable canonical GraphSnapshots, and requires two deterministic local/remote traversal fixtures to match before persisting a content-addressed activation receipt; successful remote writes maintain a complete local mirror, availability-only failure before remote observation may fall back with disclosure, and authentication, request rejection, stale/tampered/conflicting/partial state, or semantic divergence fail closed;
