...
Immediate next slice: request separate approval for one consequential Codex Run retry that validates the diagnosed large-event boundary fix through the established `ExecutionAuthorization`, external operational-state boundary, durable at-most-once lease, native descendant-tree ownership, JSONL event normalization, structured result, isolated workspace write, ResultPacket application, and Fresh HEAD transition. The approved post-hardening live Session has conformed successfully. The following approved live Run invoked Codex and observed a structured result but ended before `turn.completed`; therefore file-write verification and canonical application remain unproved. Without another model call, a deterministic 160 KiB tool-event fixture now reproduces the same failure shape under the former 128 KiB event bound and completes under the bounded 1 MiB default while retaining no raw payload. This is strong causal evidence but not retroactive proof of the historical event. The deterministic Run-draft application path accepts only a completed, native-supervised actual-provider Run and remains fail closed. The integrated ArcadeDB activation and prepared-traversal evaluation remains a separate optional `sandbox` E2E that requires explicit risk-informed external-write approval. Provider-session resume, general stream/interrupt/close, messaging, shell interpretation, and TUI scraping remain disabled until Run lifecycle conformance is proved.
...
| exact project/caller fencing | project, Session, authorization, lease, and operational-root boundaries exist; live Session passed | diagnose and prove the remaining live Run fence |
...
- six complete success/failure fixtures cover multilingual and Unicode evidence, exclusions, managed projections, invalid roots, file/byte limits, native PID cleanup, and identical JavaScript/Go request, result, response, and scan identities;
...
- cross-platform process-tree supervision is now implemented as a separate integrity-verified native helper. Windows uses a Job Object with kill-on-close and POSIX uses an isolated process group; operational control files and PIDs stay outside project canon, while durable receipts retain only the helper-manifest digest and bounded ownership/cleanup facts. Windows normal-exit and cancellation fixtures prove removal of a lingering grandchild, and the Codex protocol fixture passes through the same supervisor. A fail-closed, idempotent application bridge now maps only a completed actual-provider Run draft into canonical ResultPacket and Fresh HEAD state. The approved hardened-wire retry produced the first successful live Session with structured output, unchanged read-only fixture, and verified cleanup. Its following live Run observed structured output but terminated as `invalid-event` before `turn.completed`; file-write assertion, accepted ResultPacket application, and Fresh HEAD transition remain unproved. Lifecycle receipt `0.6.0` and draft `0.5.0` preserve only privacy-reduced diagnostic codes. A 160 KiB JSONL tool-event fixture reproduces that live failure shape under the former 128 KiB bound and completes under a 1 MiB per-event default capped by the total stdout budget, without persisting raw accepted or rejected payload. OpenCode invocation, provider resume/attachment, messaging, and general runtime controls remain deferred.
...
- 2026-08-19: diagnosed the live Run failure shape without another model call. The failed Run emitted 166,818 stdout bytes while its omitted `maxEventBytes` field inherited the former 128 KiB default. A supervised deterministic fixture now emits a valid structured result followed by one 160 KiB `item.completed` tool record: the former explicit bound reproduces `structuredResultObserved: true`, `invalid-event`, `codex.event-byte-limit`, and absent `turn.completed`; the new 1 MiB default completes the identical sequence. A smaller selected total stdout limit automatically caps the default event limit, the independent 128 KiB structured-result contract is unchanged, and accepted or rejected raw event payloads remain unpersisted. Because the historical raw JSONL was intentionally ephemeral, this is strong causal reproduction rather than a retroactive exact-event claim; one separately approved live Run remains the conformance gate.
