...
Immediate next slice: complete direct original-author source audit of the provider-neutral live-coordination vertical. Host-issued role bindings, Project/HEAD Session/generation fences, durable host-local inbox/read/reply/idempotency state, append-only binding-scoped endpoint targets, fresh-snapshot caller/target verification, exact delivery acknowledgment, post-send topology verification, and delivery-separated non-authoritative receipts are implemented. Deterministic two-fresh-process Codex/OpenCode delivery proves the portable Core path. Herdr-specific executable, socket, CLI-command, pane, and TUI integration is intentionally not part of this plugin; the same functional outcome must be reached through the generic injected `WorkspaceHostDriver` contract or a separately owned optional host adapter. Actual host-specific adapter and live-host E2E remain open acceptance work. Provider-session resume, general stream/interrupt/close, broader process-host control, shell interpretation, TUI scraping, automatic merge, automatic product-canon promotion, autonomous success judgment, and universal-directory publication remain separate optional work.
