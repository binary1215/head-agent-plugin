...
Immediate next slice: complete direct original-author source audit of the now-passing actual Codex/OpenCode provider-client composition over the provider-neutral live-coordination vertical. Host-issued role bindings, Project/HEAD Session/generation fences, durable host-local inbox/read/reply/idempotency state, append-only binding-scoped endpoint targets, fresh-snapshot caller/target verification, exact delivery acknowledgment, post-send topology verification, and delivery-separated non-authoritative receipts are implemented. A production `host-export` adapter reaches the same wake-delivery boundary without Herdr: an external host publishes content-verified endpoint snapshots whose unique endpoint tuple is bound to the current coordination `bindingId` and a host-issued per-process proof hash. A sender that owns its exact binding/proof may resolve the current recipient binding to one unique offline registered endpoint without granting that endpoint role or process authority, create a request bound to that recipient binding, and wait while the host acquires a create-only pre-effect claim. The host then starts the actual recipient provider with its distinct raw proof; the recipient's first MCP call independently revalidates proof possession and current binding ownership, reads and replies durably, and only then receives an exact create-only acknowledgment. Copied endpoint coordinates, a different or replaced binding, a forged/old proof, duplicate ownership, explicit detach, and stale topology fail closed. Actual OpenCode `send` and newly started Codex `read-inbox`/`reply` calls now pass this full path under native descendant-tree supervision, preserve byte-identical `.head`, and persist neither raw proofs/binding tokens nor provider-session references. Herdr-specific executable, socket, CLI-command, pane, and TUI integration remains intentionally outside this plugin. Provider-session resume, general stream/interrupt/close, broader process-host control, shell interpretation, TUI scraping, automatic merge, automatic product-canon promotion, autonomous success judgment, universal-directory publication, and the still-required author source verdict remain separate work.
...
| live role coordination across workspace hosts | durable role state, exact endpoint targets, per-process proof plus current-binding ownership, binding-scoped offline endpoint resolution, a production host-export filesystem bridge, and actual OpenCode send → claimed host request → newly started Codex read/reply → durable-reply-bound exact ack pass without Herdr or project mutation | **verified** |
...
- 2026-08-20: completed the Herdr-independent actual provider-client wake and role round trip. A real OpenCode 1.17.18 client called `head_coordination_send_message`; because a never-attached recipient could not previously produce a host request, the production adapter now treats the trusted snapshot as authority-free offline reachability, resolves only the exact current recipient binding, and binds that identity into request/claim/ack verification. The host acquired the create-only claim before starting a new Codex 0.144.3 HEAD process, whose distinct raw proof and current binding were revalidated before its actual `read-inbox` and immutable `reply` calls; durable reply verification preceded exact acknowledgment and the waiting OpenCode call then completed. Windows Job Objects proved descendant ownership and cleanup for both clients, `.head` remained byte-identical, actual provider thread/session references were hashed only in the ephemeral verifier and absent from scoped state, and raw proofs/binding tokens were absent from all scoped files. Explicit detach still suppresses offline wake, and copied/foreign/stale binding or proof states remain fail closed. This is direct local implementation evidence, not original-author approval; the exact commit and tests still require source audit before any comparative-superiority verdict.
