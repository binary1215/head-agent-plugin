...
- privacy-safe storage selection accepts local operation or a pending GraphDB endpoint/database plus environment-style username/password secret-reference names, rejects embedded or inline credential values, never claims remote success, and keeps the verified local path complete;
...
- incremental refresh protocol `0.1.0` now binds a verified base World Model and SourceSnapshot to immutable request/receipt evidence, rediscovers and byte-hashes the complete eligible file set, reuses semantic analysis only for digest-identical verified files, and preserves the exact complete `repository.scan.v1` result semantics;
...
- active Runs retain their accepted WholePlanSnapshot, ExecutionContract, and ContextCapsule; refresh records the pinned and refreshed SourceSnapshot identities plus a required explicit HEAD continue/recompile/revise/cancel choice, while automatic document regeneration remains disconnected;
- refresh-trigger protocol `0.1.0` now normalizes foreground filesystem and structured CI observations into bounded content-derived batches, canonically coalesces duplicates, records excluded and overflow evidence, and treats every event path only as a reason to run the complete verified scan;
...
- CLI exposes structured CI ingestion, a foreground debounced filesystem watcher, trigger status, and delivery reads; MCP remains read-only, `.head` feedback does not self-trigger, watcher summaries are bounded, active Run inputs stay frozen, and documents are not regenerated;
- inferred commit-to-ChangeSet matching, dedicated imported-backlog adapters, conformance, general candidate-promotion beyond Feature mapping and Change impact, complete execution-lineage graph projection, the remote GraphDB adapter, DocumentChangeCandidate review/application, projection of document review receipts into later GraphSnapshots, Obsidian/Notion adapters, automatic document regeneration, background watcher service installation, provider-specific CI webhook adapters, compute-backed graph/traversal/Context operations, production selection or transport amortization for the conformant Go `repository.scan.v1` candidate, descendant-tree supervision beyond the enforced no-descendant worker contract, AST-accurate/dynamic call resolution, live runtime probing/control, and authorized general knowledge promotion remain explicitly deferred.
