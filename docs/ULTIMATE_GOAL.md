...
Current milestone: v0.7 plugin-native conversation onboarding and verified incremental GraphDB data plane

Immediate next slice: the conversation-native onboarding vertical, typed GraphDB compatibility/initialization/activation path, verified incremental GraphDB write path, one-read prepared traversal, and generated Git-backed Codex marketplace distribution are implemented. Fresh NeoPick acceptance now covers no-change, small-delta, interruption/recovery, pointer-conflict/recovery, read, prepared traversal, semantic parity, credential non-persistence, unrelated-record preservation, and exact cleanup of the isolated acceptance namespace. The live run found and closed bridge graceful-exit, portable CAS, and parallel semantic-edge defects. The next optimization slice must begin from the measured dominant cost: no-change activation still spends roughly nine minutes reconstructing and independently conformance-checking the production-sized graph despite transferring zero batches. Batch size/concurrency and remaining traversal latency are tuned only from measured evidence, without skipping periodic full audit or weakening the staged-pointer trust boundaries. The marketplace snapshot reuses the immutable distribution allowlist and digest manifest, is published only after integrated tests plus all five native builds, and refuses to replace a branch that is not already a verified generated snapshot. OpenAI universal directory submission remains a separate publisher-owned review, not an engineering completion proxy. OpenCode provider authority remains in each user's resolved global configuration and authentication; this machine and NeoPick are integration fixtures, never product presets or unique authority. Provider-session resume, general stream/interrupt/close, messaging, shell interpretation, TUI scraping, automatic merge, and universal-directory publication remain separate optional work.
