...
Immediate next slice: close the original HEAD Core's remaining live-coordination advantage without importing provider, pane, or TUI identity into Core. The implemented role-coordination foundation uses host-issued role bindings, Project/HEAD Session/generation fences, durable host-local inbox/read/reply/idempotency state, and delivery-separated receipts whose authority flags remain false. Its next gate is integrated regression and original-author direct source audit, followed by an actual multi-role Codex/OpenCode native-host or Herdr adapter E2E. Provider-session resume, general stream/interrupt/close, live workspace-host attachment/delivery, shell interpretation, TUI scraping, automatic merge, automatic product-canon promotion, autonomous success judgment, and universal-directory publication remain separate optional work.
...
| durable role coordination | host-issued bindings, authority generation, durable inbox/read/reply/idempotency state, three role-bound MCP operations, and delivery-separated receipts are implemented host-locally | retain the non-authoritative Core contract and add actual multi-role live host delivery without provider identity becoming Core identity |
...
| durable role messaging | adopt as an optional host-local provider-neutral Core after the completed Session/Run vertical; live host delivery remains adapter work | messaging stays project/HEAD-Session/generation scoped, sender role is host-bound rather than self-claimed, and no message creates a new authority identity |
