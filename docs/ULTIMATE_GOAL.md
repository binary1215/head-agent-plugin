...
Current milestone: v0.8 minimal Product Operating Loop and portable derived HEAD continuity

Immediate next slice: the minimal Product Operating Loop is implemented as immutable epistemically typed Signal, Hypothesis, Initiative candidate/review, reviewed Initiative, Feature resolution, and execution-bound OutcomeObservation artifacts projected into the local World Model. `HEADContinuitySnapshot` is an on-demand non-persisted exact-reference view; Session/Run checkpoints remain recovery authority and Product Canon remains user-owned. Integrated validation and author re-review confirmed the candidate-immutability and authority boundary with no remaining blocking correction; GraphDB activation was not part of this slice. The measured GraphDB optimization is now the next performance slice: no-change activation still spends roughly nine minutes reconstructing and independently conformance-checking the production-sized graph despite transferring zero batches. Optimization must use measured evidence without weakening periodic audit, staged-pointer trust, credential safety, or exact-target boundaries. Provider-session resume, general stream/interrupt/close, messaging, shell interpretation, TUI scraping, automatic merge, automatic product-canon promotion, autonomous success judgment, and universal-directory publication remain separate optional work.
...
Product learning extends the Product Graph without becoming the HEAD control plane. The minimum active flow is `ProductSignal(observed-fact) -> ProductHypothesis(hypothesis) -> ProductInitiativeCandidate(inferred-meaning) -> explicit user ReviewDecision -> ReviewedProductInitiative(approved-decision)`. A reviewed Initiative may link to an exact current Feature, a separate non-canonical ProductFeatureCandidate, or an explicit mapping gap. It does not mutate Product Canon.
