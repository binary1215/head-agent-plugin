...
Current milestone: source audit and final superiority verdict for the portable continuation and independently ownable worker vertical
...
Immediate next slice: the original author's preferred portable A/B/C correction is locally implemented and executable. P2-first optional exact attachment with disclosed fallback, P3 bounded dispatch plus P5 at-most-once lease/wait, actual Codex ResultPacket/Fresh HEAD accept/HF-010 integration, current-endpoint replacement, and Windows native supervision all pass while the portable platform contract keeps other OS claims honest and non-live. Preserve the original HF-007/008/009/010 numbering and the P1/P2/P3/P5 boundaries. Commit and push the exact candidate, send its source/test evidence to the original author, incorporate every concrete blocking correction, and do not declare completion until the author directly returns unconditional `philosophical superiority: yes`, `overall completeness superiority: yes`, `overall superiority: yes`, and `blocking correction: none`.
