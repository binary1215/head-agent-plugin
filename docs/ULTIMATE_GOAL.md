...
Immediate next slice: commit and push the provider-neutral corrections, run the opt-in actual Codex/OpenCode verifier, and return the exact source commit plus evidence to the original author. The implementation starts Codex HEAD first and attaches the current endpoint while it boundedly waits; OpenCode then starts under a distinct process proof, sends an authority question, and boundedly waits for HEAD's immutable reply. The host claims and acknowledges the current endpoint without spawning a provider, and a prior replaced endpoint must receive zero requests. Reply wait remains independent of delivery acknowledgment and creates no `ReviewDecision`. The exact-owned one-shot supervisor activates only token-fenced `interrupt` and `close`, with real Codex/OpenCode cleanup evidence; provider-session resume, stream, service hosting, and broader process-host control remain deferred. Raw process proofs, binding/control tokens, provider-session identities, prompts, and transcripts remain outside persisted state; `.head`, Product Canon, compaction canon, and execution authority are unchanged. No Herdr executable, socket, command, pane, or TUI behavior enters this plugin; equivalent coordination remains a provider-neutral host contract. Final completion still requires direct author source/test inspection, no blocking correction, and explicit `philosophical superiority: yes`, `overall completeness superiority: yes`, and `overall superiority: yes`.
...
- 2026-08-21: the actual original author directly re-audited pushed commit `cbc49bc` and reported no blocking correction, while withholding all three final superiority verdicts for three precise evidence gaps: actual already-running Codex/OpenCode exact attachment plus endpoint replacement, bounded one-shot interrupt/close, and a worker authority-question round trip that waits for HEAD without treating its answer as a ReviewDecision. The provider-neutral correction adds bounded inbox/reply waits that reauthenticate on every poll; reply observation is delivery-independent and explicitly non-authoritative. The production live verifier now starts Codex HEAD before the OpenCode worker, proves distinct provider children and current endpoint replacement, performs request claim/ack without spawn-on-claim, requires zero stale-endpoint requests, and makes OpenCode wait for the exact immutable answer. Separate real Codex/OpenCode clients exercise token-fenced one-shot interrupt and close under native descendant ownership, while resume and stream remain disabled. This entry records implementation direction and local deterministic evidence only; the new actual model E2E, pushed commit, and renewed direct author verdict remain pending.
