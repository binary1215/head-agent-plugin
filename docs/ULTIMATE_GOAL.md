...
Immediate next slice: add deterministic transport-cost and latency evidence for prepared traversal, then validate the same contract against a live-compatible ArcadeDB environment without mutating authority, weakening full status audits, or making GraphDB required.
