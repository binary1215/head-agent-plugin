...
Immediate next slice: execute the integrated graph activation plus read-only prepared-traversal flow only against the user-designated ArcadeDB `sandbox` after explicit risk-informed external-write approval, then bind a future provider invocation to an accepted ExecutionContract with exact caller/child ownership and bounded event/ResultPacket schemas. Provider-session start, resume, stream, interrupt, close, messaging, shell interpretation, and TUI scraping remain disabled until that lifecycle conformance is proved. The live GraphDB evaluation remains derived operational evidence rather than a core prerequisite.
...
- provider-session hydration, actual provider-session project binding, caller and process-tree fencing, bounded event/ResultPacket schemas, messaging, and every runtime control operation remain deferred until ExecutionContract-bound lifecycle conformance is proved.
...
The contract foundation defines and tests `PlatformAdapter`, `AgentRuntimeAdapter`, and `WorkspaceHostAdapter` references for Codex and OpenCode across Windows, macOS, Linux, and the native-process host. Privacy-preserving current-host CLI discovery, bounded direct non-session version evidence, provider-specific fixed-help protocol/capability evidence, and canonical HEAD project/Session capability binding are active without raw path, command, output, or project-content disclosure, and the current Windows host verifies both runtimes. Next, require an accepted ExecutionContract and prove exact caller/child ownership, project fencing, bounded provider event and ResultPacket schemas, cancellation/close, and cleanup before enabling worker launch, resume, stream, interrupt, close, messaging, or process control. Capability discovery never grants authorization, and provider sessions never replace HEAD Session or Run identities.
