...
| --- | --- | --- |
| initialization and resumable phases | installed `init`/`resume` composes project verification, managed release-path convergence, and onboarding start/recovery; repeated pending-review and ready calls are non-duplicating | retain this public path while adding final cross-provider recovery evidence |
...
| immutable release and rollback | user-scoped plugin releases are content-identified, staged, verified, activated by a recoverable pointer, and explicitly rollback-capable | retain installed-cache immutability while adding later marketplace/native artifact adapters |
