...
Immediate next slice: complete the OpenCode one-shot adapter through the same authorization, lease, native process-tree supervision, structured-result, and invocation-record core already used by Codex. Then finish the immutable install/upgrade/rollback path and product-facing README before the integrated E2E milestone. For the current remote acceptance run, the user has explicitly selected the `neopick` database rather than `sandbox`; perform a compatibility audit first, preserve unrelated schema, and reset the selected database only if incompatible reserved `HeadAgentGraph*` data is proven to block conformance. The retained isolated NeoPick onboarding Session contains 23 evidence-linked candidates and a bounded `revise` proposal; no Product Canon promotion is implied by remote graph projection. Provider-session resume, general stream/interrupt/close, messaging, shell interpretation, TUI scraping, and automatic merge remain disabled.
