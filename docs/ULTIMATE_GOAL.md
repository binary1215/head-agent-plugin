...
Immediate next slice: define a provider-neutral prepared traversal request that carries the already-fixed GraphSnapshot, anchors, policy, and bounds to an activated backend, so ArcadeDB can avoid full remote snapshot/topology reload without changing client canonicalization, semantic digests, fallback rules, or authority.
