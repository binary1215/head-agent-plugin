...
Runtime observations have neither instruction nor control authority and do not hydrate a provider session. Temporal graph nodes and edges remain rebuildable evidence without canon, instruction, or promotion authority. `ProductContext` is a derived projection of user-owned Product Canon and cannot mutate it. The onboarding state machine is active, but unreviewed onboarding candidates remain outside normal Context Capsules; only the resulting reviewed Product Canon projection may compete for context. The current semantic graph and Symbol extraction are heuristic rather than AST-accurate. ChangeSet and product-to-code graph projection, candidate/receipt graph projection, structured decision inference and supersession from Git evidence, live runtime probing/control, cross-repository resolution, document projection, and the optional GraphDB adapter remain explicit Unknowns.
