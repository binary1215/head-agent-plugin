...
Runtime observations have neither instruction nor control authority and do not hydrate a provider session. Temporal graph nodes and edges remain rebuildable evidence without canon, instruction, or promotion authority. `ProductContext` is a derived projection of user-owned Product Canon and current explicitly reviewed Feature mappings and cannot mutate either source. Normal Context compilation never opts into onboarding or mapping candidate surfaces. The current semantic graph, Symbol extraction, and lexical mapping inference are heuristic rather than AST-accurate. ChangeSet projection, general candidate promotion, structured decision inference and supersession from Git evidence, live runtime probing/control, cross-repository resolution, document projection, and the optional GraphDB adapter remain explicit Unknowns.
