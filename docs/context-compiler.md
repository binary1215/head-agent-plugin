...
Runtime observations have neither instruction nor control authority and do not hydrate a provider session. Temporal graph nodes and edges remain rebuildable evidence without canon, instruction, or promotion authority. `ProductContext` is a derived projection of user-owned Product Canon plus current explicitly reviewed Feature mappings and Change impacts and cannot mutate any source. Normal Context compilation never opts into onboarding, mapping, or impact candidate surfaces. The current semantic graph, Symbol extraction, lexical mapping inference, and mapping-based impact inference are heuristic rather than AST-accurate. VcsEvidence attachment, general candidate promotion, structured decision inference and supersession from Git evidence, live runtime probing/control, cross-repository resolution, document projection, and the optional GraphDB adapter remain explicit Unknowns.
