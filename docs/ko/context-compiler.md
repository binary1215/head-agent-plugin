...
Compiler는 “이 executor가 이 작업을 수행하기 위해 무엇을 알아야 하며, 각 항목은 왜 포함되었는가?”라는 질문에 답합니다. 최대 context가 아니라 최소 충분 context를 최적화합니다.
...
Context Compiler protocol `0.12.0`은 model call 없이 Capsule을 재현할 수 있도록 결정적이며 coverage를 인식하는 lexical ranker를 사용합니다. URL payload를 검색 term에서 제외하면서 Unicode, camelCase, snake_case, 저장소 path, 제한된 한국어 조사 변형 집합을 정규화합니다. 저장소 검색은 먼저 가벼운 file/symbol/dependency 증거의 순위를 매기고, budget에서 파생된 lexical seed 집합과 그 직접 graph neighbor에서 semantic adjacency를 확장하며, 저장소 file context를 위해 최대 한 번의 결정적 temporal anchor traversal을 수행합니다. ProductContext는 별도의 단일 bounded traversal로 유지됩니다. 향후 같은 data contract 뒤에 model-assisted compilation을 추가할 수 있습니다.

명시적 budget은 엄격한 상한이며 충분하다는 주장이 아닙니다. HEAD는 현재 작업에 필요한 evidence kind, facet, relation type과 최소 item 수를 명명하는 작업 로컬 `EvidenceNeed[]` contract를 제공할 수 있습니다. Compiler는 후보가 있다는 이유로 이러한 need를 만들어 내지 않으며, 보편적인 source, test, ProductContext 또는 graph-neighborhood 요건도 강제하지 않습니다. 제공된 need를 위해 budget을 확보하고, 일치하는 증거가 선택된 Capsule에 실제로 존재하는지만 입증합니다.
