...
제한보다 먼저 확보하므로, 어휘가 겹치지 않아도 정확히 요청한 관계가 숨겨지지
않습니다. Capsule은 여전히 선언된 예산 안에서 구성하며 누락을 표시합니다.
Repository carrier와 coverage proof는 `representation.kind: repository-metadata`,
`sourceBodyIncluded: false`, `sourceBodyConsumptionVerified: false`를 명시합니다.
심볼이나 관계가 포함됐다는 사실은 소스 본문이 포함되거나 읽혔다는 증명이 아닙니다.
HEAD는 의미 판단이나 구현 판단 전에 필요한 소스를 직접 확인해야 합니다.

선택 이득과 추가 예산 추정은 미충족 need별로 새로 포함되는 evidence ID만 셉니다.
같은 관계를 반복하는 carrier가 두 수치를 부풀릴 수는 없지만, 별도의 repository/source
need를 충족한다면 계속 선택할 수 있습니다. 대체 carrier도 proof에 표시합니다.
이는 집합 계산이며 의미적 관련성 판단이나 새로운 실행 게이트가 아닙니다.

Product coverage는 ProductContext carrier 수가 아니라 검증된 Project, Product Model,
논리적 entity(kind와 key), revision identity를 셉니다. 정확한 key와 facet-only need
모두 선택 이득·coverage·예산 추정에 같은 identity를 사용합니다. 반복 carrier의
출처는 별도로 보존하지만 개수는 부풀리지 않습니다. HEAD가 정확히 요청한 Product
key는 선택적 이웃 순위보다 먼저 확인하고 discovery 제한에 앞서 보존합니다.
없는 key는 계속 미충족이며 요청한 evidence에도 Capsule 예산은 적용됩니다.
이는 의미적 충분성 판단이나 candidate 승인이 아닙니다.

Product 요구는 함께 사용할 수 있습니다. 정확한 key의 후보와 각 facet-only 요구를
위한 제한된 어휘 탐색 후보를 합칩니다. 정확한 요구를 하나 추가해도 다른 요구의
