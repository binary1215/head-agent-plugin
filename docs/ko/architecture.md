...
World Model은 지원되는 저장소 파일, 휴리스틱 심볼, 종속성, import 및 해석 가능한 호출을 콘텐츠 주소 지정 증거로 구체화합니다. Hot은 현재 스냅샷이고, Warm은 포인터의 변경 요약이며, Cold는 보존된 이전 스냅샷 집합입니다. Context Compiler가 후보 또는 그래프 쿼리를 소비하기 전에 작업 트리를 기준으로 파일 수준 최신성을 다시 계산합니다.
...
semantic graph는 콘텐츠에서 파생된 node 및 edge ID, 파일 다이제스트 및 line 증거, 명시적인 휴리스틱 신뢰도, `evidence-not-instruction` 신뢰 경계를 사용합니다. 현재 파일 포함, 모듈 import 및 해석 가능한 JavaScript/TypeScript/Python 호출을 다룹니다.
...
temporal provenance graph는 별도의 검증된 프로젝션이므로 기존의 휴리스틱 semantic ID가 자동으로 재정의되지 않습니다. 안정적인 Repository/File/Symbol/Test 및 제품 논리 ID를 불변 revisions와 분리하고, 명시적인 0개 이상의 SourceSnapshot, Revision 및 ChangeSet 부모를 지원하며, 출처가 완전한 typed edge를 검증합니다. Product node와 Product relation은 `canon-projected`입니다. 온보딩, Feature-mapping, ChangeSet, Change-impact 및 문서 검토 아티팩트는 Canon이 되지 않으면서 검토 추적을 보존합니다. 후보 공간 node는 명시적으로 요청하지 않는 한 일반 순회에서 제외되며, 검토된 영수증과 현재 검토된 `IMPLEMENTS`, `VERIFIED_BY` 및 `IMPACTS` edge는 계속 쿼리할 수 있습니다. ChangeSets는 Git 없이도 정확한 변경 전후 소스 증거를 수락된 ResultPacket/ReviewDecision 리니지에 바인딩합니다. 선택적 불변 VCS 증거는 검증된 Git history에서 선택한 commit 관찰을 내장하고 `MATERIALIZED_AS` / `REFERENCES`를 프로젝션합니다. 이는 ChangeSet ID를 절대 변경하지 않으며 실제 Git이 사라진 후에도 재구축할 수 있습니다. 그래프는 파생 상태로 유지되며 범위가 지정된 ReviewDecision 명령 없이는 Canon을 변경하거나 후보를 승격할 수 없습니다. 범위가 한정된 `TraversalQuery`는 relation, kind, authority, freshness, confidence, depth, node, edge, candidate 포함 및 ordering policy를 고정하고 graph/query/result 다이제스트를 반환합니다. 로컬 및 활성화된 ArcadeDB 프로젝션은 내장 GraphSnapshot을 복구 가능한 소스로 보존합니다. 추론된 commit 매칭, 활성 범위를 넘어서는 일반적인 권한 있는 승격, 적합성, 자동 merge, compare-and-swap 원격 게시 및 Obsidian/Notion 프로젝션은 계속 연기됩니다.
