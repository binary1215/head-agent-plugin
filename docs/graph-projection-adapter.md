...
The 2026-08-20 privacy-safe live acceptance used an already verified 8,037-node and 12,441-edge project graph. Seven identical prepared traversals processed 220,930 normalized bytes instead of the 23,101,697-byte full-reload baseline, saving 22,880,767 bytes or 9,904 basis points. Every run preserved the exact graph, request, query, and result identities; query-phase writes, full snapshot reads, and full topology reads remained zero. End-to-end median latency was 2,942.2 ms while the bounded database expansion itself was 134.7 ms median. The next latency slice therefore targets duplicate pointer reads and repeated local canonical verification before considering database-query changes. These timings are operational diagnostics, not semantic identity.
...
- manifest-diffed, resumable delta activation with content-addressed node/edge batches, idempotent checkpoints, adaptive concurrency, bounded retry/backoff, remote digest/count verification, and atomic verified-pointer advance;
- duplicate pointer-read elimination and amortized local canonical verification, measured independently from database expansion latency;
