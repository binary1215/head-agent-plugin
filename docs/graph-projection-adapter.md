...
The 2026-08-20 privacy-safe live acceptance used an already verified 8,037-node and 12,441-edge project graph. Seven identical prepared traversals processed 220,930 normalized bytes instead of the 23,101,697-byte full-reload baseline, saving 22,880,767 bytes or 9,904 basis points. Every run preserved the exact graph, request, query, and result identities; query-phase writes, full snapshot reads, and full topology reads remained zero. End-to-end median latency was 2,942.2 ms while the bounded database expansion itself was 134.7 ms median. Those measurements identified duplicate pointer reads and repeated local canonical verification as the next client-side targets. The subsequent reviewed fixture keeps the exact same graph, request, result, and cost-evidence identities while seven queries reduce pointer reads from 14 to seven and normalized pointer-response bytes from 10,052 to 5,026. Redundant validations inside already-verified response/build helpers were removed, but construction, adapter entry, and client receipt boundaries still verify independently. The next client-receipt slice removes one full request revalidation and one locally constructed expected-receipt self-validation per query: the same seven-query fixture records seven independent receipt-digest checks, seven exact bindings to the untouched same-stack locally built request, and zero full request reverifications at the receipt boundary. A valid receipt replayed from a different request is still rejected. Fixture timing is not used to claim a live latency improvement; that requires a fresh credential-backed NeoPick run. These timings and counts are operational diagnostics, not semantic identity.
