...
This version activates project initialization, project-scoped immutable HEAD Session records, explicit onboarding state, privacy-safe local or pending GraphDB storage selection, bounded evidence-linked onboarding candidate sets, batch onboarding ReviewDecisions, ReviewDecision-gated Product Canon bootstrap, Feature/code/test mapping CandidateSets and explicit mapping ReviewDecisions, separate reviewed `IMPLEMENTS`/`VERIFIED_BY` promotion, provider-neutral reviewed ChangeSets with exact revision deltas and multiple-parent DAG shape, immutable Change-impact candidates and explicit review-gated `IMPACTS` promotion, optional explicit ChangeSet-to-VCS evidence, deterministic Context Capsule compilation, content-derived Execution Lineage artifacts, contract-bound Runs, Result Packets, deterministic Fresh HEAD review projections, strict Product Model canon, incremental Repository World Model and refresh flows, replaceable graph/document/compute adapters, verified Go transport, and provider-neutral runtime contracts. Runtime coverage now includes privacy-preserving CLI discovery, bounded version/help evidence, HEAD project/Session capability binding, Session/Run `ExecutionAuthorization`, external host-local operational state, durable project-lineage at-most-once receipts, request/input drift and replay rejection, privacy-reduced provider diagnostics, bounded structured results, integrity-verified native process-tree supervision, and a supervised Codex exec JSONL protocol fixture with durable CLI/MCP retrieval. Windows Job Object normal-exit/cancellation cleanup is executed locally and the POSIX process-group implementation is built and run in CI. The actual Codex one-shot composition has passed both the bounded live Session and consequential live Run, including isolated write, canonical ResultPacket application, and Fresh HEAD review. NeoPick-quality source scoping, behavior-clustered onboarding inference, and bounded Context compilation are locally conformed. The current 23 candidates remain unreviewed; use the exact proposal in `../../docs/neopick-onboarding-review-proposal.md` for user review, never `accept-all`. Live `neopick` database activation remains unexecuted. Do not promote candidates or claim remote conformance from local evidence. OpenCode invocation, provider resume/attachment, general runtime start/stream/interrupt/close, provider runtime hydration, role messaging, service installation, Herdr, and the other deferred capabilities remain unavailable. Do not represent those deferred capabilities as working.
