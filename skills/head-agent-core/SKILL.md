...
Read `../../docs/runtime-adapters.md` before inspecting or extending runtime integration. `runtime-adapters` may execute only the fixed version and help profiles defined by the provider-neutral boundary, return only normalized capability signals and digest/lifecycle evidence, and bind them to canonical HEAD project and Session identities without passing project content. Capability observation and HEAD binding do not authorize or create a provider session. Keep `start`, `resume`, `stream`, `interrupt`, `close`, host messaging, and process control disabled until an accepted ExecutionContract, exact caller/child ownership, bounded event and ResultPacket schemas, cancellation/cleanup, and actual provider-session evidence pass conformance.
...
This version activates project initialization, project-scoped immutable HEAD Session records, explicit onboarding state, privacy-safe local or pending GraphDB storage selection, bounded evidence-linked onboarding candidate sets, batch onboarding ReviewDecisions, ReviewDecision-gated Product Canon bootstrap, Feature/code/test mapping CandidateSets and explicit mapping ReviewDecisions, separate reviewed `IMPLEMENTS`/`VERIFIED_BY` promotion, provider-neutral reviewed ChangeSets with exact revision deltas and multiple-parent DAG shape, immutable Change-impact candidates and explicit review-gated `IMPACTS` promotion, optional explicit ChangeSet-to-VCS evidence with embedded Git commit observations and `MATERIALIZED_AS`/`REFERENCES` projection, candidate/review/promotion receipt GraphSnapshot projection, verified post-promotion GraphSnapshot rebuild, runtime projection, canonical Session/Run state, checkpoints, deterministic Context Capsule compilation, content-derived Execution Lineage artifacts, contract-bound Runs, Result Packets, deterministic Fresh HEAD review projections, manual HEAD review gates, ReviewDecision-linked plan generations, authority-free knowledge proposals, strict user-owned Product Model canon, an incremental Repository World Model alpha, explicit incremental refresh requests and receipts, digest-gated unchanged-file analysis reuse, automatic SourceSnapshot/revision ancestry, active-Run refresh drift disclosure, bounded debounced filesystem and structured CI trigger ingestion, immutable trigger batches/delivery receipts, project-scoped single-writer refresh serialization, replaceable World Model/Git-history/runtime-state adapter contracts, Codex/OpenCode `AgentRuntimeAdapter`, Windows/macOS/Linux `PlatformAdapter`, and native-process `WorkspaceHostAdapter` contract references plus privacy-preserving current-host CLI discovery, bounded non-session version and fixed-help protocol evidence, and canonical HEAD project/Session capability binding, a versioned GraphProjectionAdapter with local JSON, in-memory, and conformance-gated ArcadeDB HTTP implementations, environment-only credential resolution, immutable remote snapshots, verified pointers, snapshot-scoped topology, bounded server expansion, prepared traversal verification, local mirroring, adapter-neutral temporal traversal, and disclosed availability fallback, a versioned DocumentProjectionAdapter with deterministic Markdown and local/in-memory conformance, published-view drift protection, immutable DocumentChangeCandidate capture, explicit document-change review, exact structured Product Canon application, child GraphSnapshot rebuild, deterministic Markdown reconciliation, immutable application receipts, all-reachable commit-message evidence, strict point-in-time runtime evidence, heuristic semantic edges, a Git-independent temporal Product/File/Symbol/Test revision GraphSnapshot with multiple-parent DAG support, bounded ProductContext and semantic/temporal/history/runtime traversal, freshness-gated context, a versioned ComputeAdapter/WorkerProtocol contract, validated `repository.scan.v1` JavaScript reference operation, tracked conformance and benchmark corpus, a complete-response-conformant Go repository-scan candidate, verified Go stdio transport, content-addressed platform manifest selection, bounded exact-child lifecycle, and disclosed JavaScript fallback. It does not yet provide background watcher service installation, provider-specific CI webhooks, dedicated imported-backlog adapters, inferred commit-to-ChangeSet matching, general relationship promotion beyond Feature mapping and Change impact, automatic document regeneration beyond the active safe Markdown path, Obsidian/Notion projection, compute-backed graph/traversal/Context operations, production selection or transport amortization for Go `repository.scan.v1`, descendant-tree supervision beyond the no-descendant worker contract, automatic ChangeSet ancestry inference, automatic merge/conflict resolution, AST-accurate semantic analysis, structured or promoted decision extraction from Git evidence, actual provider-session project attachment, invocation, or control beyond capability-reference binding, an executed live ArcadeDB prepared-query evaluation, compare-and-swap remote publication, non-ArcadeDB graph transports, provider runtime hydration, authorized general knowledge promotion, live-caller fencing, role messaging, service installation, or Herdr. Do not represent those deferred capabilities as working.
