...
Read `../../docs/runtime-adapters.md` before inspecting or extending runtime integration. `runtime-adapters` may execute only the fixed version and help profiles defined by the provider-neutral boundary, return only normalized capability signals and digest/lifecycle evidence, and bind them to canonical HEAD project and Session identities without passing project content. Capability observation and HEAD binding do not authorize or create a provider session. `runtime-invocation-authorize` may only prepare an immutable authorization from the exact active Run when its ExecutionContract explicitly allows `runtime.invoke` and `project.read` or `project.write`; it must not execute a provider. Runtime execution must atomically consume that authorization at most once before child start and retain an immutable release record; never replay a consumed authorization after completion, failure, cancellation, timeout, or caller crash. `runtime-invocation-lease-status` and the matching MCP tool are read-only. Event envelopes, lifecycle receipts, and ResultPacket drafts remain transcript-free evidence. Keep `start`, `resume`, `stream`, `interrupt`, `close`, host messaging, and actual provider process control disabled until live caller/process-tree ownership, actual provider events, cancellation/cleanup, and provider-session evidence pass conformance.
