...
Safe opt-in automatic Markdown regeneration is active through the manual-default `PostRefreshProjectionPolicy`, with immutable refresh-linked receipts and edited-view candidate capture. In the limitation sentence above, automatic document regeneration now refers to unreviewed candidate application, Obsidian/Notion publication, or bidirectional synchronization; those capabilities remain deferred.
