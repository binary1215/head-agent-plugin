...
Read `../../docs/compute-adapter.md` before adding or consuming a compute operation. The JavaScript reference backend is the semantic oracle. The verified Go transport is active only for operations advertised by its integrity-checked manifest; the packaged worker currently advertises authority-free health, not `repository.scan.v1`. Do not claim native acceleration for an operation until its strict schema, deterministic reference implementation, conformance fixtures, failure cases, benchmark corpus, and native equivalence exist. A compute result has no authority effect and cannot write Product Canon, create a ReviewDecision, promote a candidate, mutate project state, or widen an ExecutionContract.
